...
class MediaControlsApplet extends Applet.TextIconApplet {
    ...
    _initPrefs() {
        this._preferences = {
            showTrackInfo: "song_artist",
            truncateText: false,
            truncateLength: 30
        };

        this.settings = new Settings.AppletSettings(this, "mediacontrols@rbfraphael", this._appletInstanceId);
    ...
    _updatePrefs() {
        this._preferences.showTrackInfo = this.settings.getValue("showTrackInfo");
        this._preferences.truncateText = this.settings.getValue("truncateText");
        this._preferences.truncateLength = this.settings.getValue("truncateLength");
        this._updateUi();
    }
    ...
    _updateUi() {
        try {
            if (this._activePlayer) {
                ...
                let songDescription = "";
                if(this._preferences.showTrackInfo != "none") {
                    if(this._preferences.showTrackInfo == "app_name") { 
                        songDescription = this._activePlayer.identity;
                    } else {
                        songDescription = this._activePlayer.songTitle;
                        if (this._activePlayer.songArtist.length > 0 && this._preferences.showTrackInfo == "song_artist") {
                            songDescription += songDescription.length > 0 ? " - " : "";
                            songDescription += this._activePlayer.songArtist;
                        }
                    }

                    if (this._preferences.truncateText) {
                        songDescription = songDescription.length > this._preferences.truncateLength ? songDescription.substring(0, this._preferences.truncateLength - 3) + "..." : songDescription;
                    }
                }
