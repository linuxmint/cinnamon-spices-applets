...
class MyWindow(Gtk.Window):
    ...
    def __init__(self):
        ...
        if (config.has_key('SHOW_BOOKMARKS')):
            ...
            switch_show_bookmarks.create(vbox_right, 'SHOW_BOOKMARKS', config['SHOW_BOOKMARKS'], "Show \"Favorites\" Label?")

	if (config.has_key('COLLAPSE_BOOKMARKS')):
            switch_drop_bookmarks = switch_option()
            switch_drop_bookmarks.create(vbox_right, 'COLLAPSE_BOOKMARKS', config['COLLAPSE_BOOKMARKS'], "Show \"Favorites\" Icon?")

        if (config.has_key('SHOW_DEVICES')):
            switch_show_devices = switch_option()
            switch_show_devices.create(vbox_right, 'SHOW_DEVICES', config['SHOW_DEVICES'], "Show \"Favorites\" Tooltips?")

        if (config.has_key('COLLAPSE_DEVICES')):
            switch_drop_devices = switch_option()
            switch_drop_devices.create(vbox_right, 'COLLAPSE_DEVICES', config['COLLAPSE_DEVICES'], "Frame the boxes?")
