...
The applet normally shows the percentage charge on a coloured background which is:
  ...
  * NEW in version 1.3.4  The user's home folder is checked for a file called batterymonitorwarning.mp3 which is then used instead of the default. When a file is found a warning is given at startup that the volume and length should be restricted in public places. Mine lasts for 30 seconds and can be heard throughout the house so is not ideal for libraries - a copy can be found in the applet folder with thanks to Brandon at http://soundbible.com/1911-Mandatory-Evacuation.html and used under Attribution 3.0 Licence.
  * When the Shutdown level is reached an alert box is put up with options of an immediate suspend or cancel - if nothing is done it suspends 30 seconds latter. If cancelled (or the machine is turned back on without the battery being on charge) the alert and shutdown sequence is restarted after the battery has dropped another 1%. The alert box is accompanied by an audible alert.
