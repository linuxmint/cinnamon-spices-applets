 ...
 *
 * Derived from Cinnamon's xapp-status@cinnamon.org applet.
 * Modified on 2026-06-24 to support persistent drag-and-drop icon ordering,
 * and repackaged as an independently installable applet on 2026-06-25.
 *
 * Copyright (C) Linux Mint and Cinnamon contributors
 *
...
class XAppStatusIcon {
    constructor(applet, proxy) {
        ...
        // Register DND before the status-icon button handler. The latter stops
        // event propagation, so DND must grab the initial press first.
        this._draggable = DND.makeDraggable(this.actor);
        this.applet.signalManager.connect(this._draggable, 'drag-begin', this.onDragBegin, this);
        this.applet.signalManager.connect(this._draggable, 'drag-end', this.onDragEnd, this);
    ...
    onDragBegin() {
        this.applet.draggingIcon = this;
        this._tooltip.hide();
        this._tooltip.preventShow = true;
    }

    onDragEnd(draggable, time, success) {
        this.applet.draggingIcon = null;
        ...
        if (iconName) {
            ...
            if (iconName.includes("/") && type != St.IconType.SYMBOLIC) {
                this.icon_loader_handle = St.TextureCache.get_default().load_image_from_file_async(
                    ...
                    this.actor.vertical ? this.iconSize * global.ui_scale : -1,
                    this.iconSize * global.ui_scale,
    ...
    _onImageLoaded(cache, handle, actor, data=null) {
        if (handle !== this.icon_loader_handle) {
            global.logError(`xstatus-applet-reloaded@dshep79: Icon or image seems out of sync (${this.name}`);
            return;
        }
...
class CinnamonXAppStatusApplet extends Applet.Applet {
    constructor(metadata, orientation, panel_height, instance_id) {
        ...
        this.manager_container._delegate = this;
        ...
        this.settings.bind("icon-order", "iconOrder", this.sortIcons);
    ...
    _sortFunc(a, b) {
        let aId = this.getOrderId(a);
        let bId = this.getOrderId(b);
        let aIndex = this.iconOrder.indexOf(aId);
        let bIndex = this.iconOrder.indexOf(bId);
    ...
    getOrderId(icon) {
        return icon.proxy.name.replace("org.x.StatusIcon.", "").toLowerCase();
    }

    handleDragOver(source, actor, x, y, time) {
        if (!(source instanceof XAppStatusIcon) || source.applet !== this) {
            return DND.DragMotionResult.NO_DROP;
        }

        let vertical = this.orientation == St.Side.LEFT || this.orientation == St.Side.RIGHT;
        let boxSize = vertical ? this.manager_container.height : this.manager_container.width;
        let pointerPosition = vertical ? y : x;
        let iconCount = Object.keys(this.statusIcons).length;

        if (boxSize <= 0 || iconCount < 2) {
            return DND.DragMotionResult.MOVE_DROP;
        }

        if (!vertical && St.Widget.get_default_direction() === St.TextDirection.RTL) {
            pointerPosition = boxSize - pointerPosition;
        }

        let dropIndex = Math.floor(pointerPosition / boxSize * iconCount);
        dropIndex = Math.max(0, Math.min(iconCount - 1, dropIndex));
        this.manager_container.set_child_at_index(source.actor, dropIndex);
    ...
    acceptDrop(source, actor, x, y, time) {
        ...
        for (let child of this.manager_container.get_children()) {
            if (child._delegate instanceof XAppStatusIcon) {
                let id = this.getOrderId(child._delegate);
        ...
        this.settings.setValue("icon-order", order);
        this.sortIcons();
        return true;
    }
    ...
    sortIcons() {
        ...
        icon_list.sort((a, b) => this._sortFunc(a, b));

        for (let index = 0; index < icon_list.length; index++) {
            this.manager_container.set_child_at_index(icon_list[index].actor, index);
        }
    ...
    on_applet_removed_from_panel() {
        ...
        this.monitor = null;
        this.manager_container._delegate = null;
    ...
    on_panel_edit_mode_changed() {
        ...
        for (let owner in this.statusIcons) {
            ...
            icon._draggable.inhibit = !reactive;
        }
    }
