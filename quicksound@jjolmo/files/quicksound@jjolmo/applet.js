...
class CinnamonSoundApplet extends Applet.TextIconApplet {
    constructor(metadata, orientation, panel_height, instanceId) {
        ...
        for (let i = 1; i <= 10; i++) {
            this.settings.bind("iconRules" + i + "Name", "iconRules" + i + "Name", this.on_settings_changed);
            this.settings.bind("iconRules" + i + "Icon", "iconRules" + i + "Icon", this.on_settings_changed);
            this.settings.bind("iconRules" + i + "Rule", "iconRules" + i + "Rule", this.on_settings_changed);
        }

        this.settings.bind("iconRulesFilterList", "iconRulesFilterList", this.on_settings_changed);
        ...
        this._control.connect('output-added', () => this._populateOutputDeviceList());
        this._control.connect('output-removed', () => this._populateOutputDeviceList());
        this._control.connect('active-output-update', () => this._populateOutputDeviceList());
    ...
    _populateOutputDeviceList() {
        ...
        devices.forEach(device => {
            ...
            this._outputDeviceItems.push(item);
        });
    }
...
class DeviceMenuItem extends PopupMenu.PopupBaseMenuItem {
    ...
    constructor(appletObj, device, showIcon) {
        ...
        for (let i = 1; i <= 8; i++) {
            ...
            let nameKey = `iconRules${i}Name`;

            if (appletObj[ruleKey] && desc.includes(appletObj[ruleKey].toLowerCase())) {
                chosenIcon = appletObj[iconKey] || chosenIcon;
                chosenName = appletObj[nameKey] || chosenName;
                break;
            }
        }
        ...
        icon.set_style("width: 32px; height: 32px;");
        ...
        if (showIcon) {
            label.set_style("color: white; font-weight: bold;");
        }
