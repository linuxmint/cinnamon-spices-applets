...
const AppletMeta = imports.ui.appletManager.applets["SettingsPlus@lusito.info"];
...
SettingsLauncher.prototype = {
    ...
    execute: function() {
        if(this._settings.exec && this._settings.exec != "") {
            ...
            do {
                if(command == "[reload-theme]") {
                    ...
                    command = command.replace('${HOME}', GLib.get_home_dir());
                    command = command.replace('${APPLET_DIR}', AppletDir);
...
SettingsPlus.prototype = {
    ...
    _init: function(orientation) {
        ...
        try {
            ...
            let settings_menu_item = new Applet.MenuItem(_("Edit Menu"), Gtk.STOCK_EDIT, Lang.bind(this, function(event) {
    ...
    load_menu: function(config) {
        ...
        for(let i=0; i<config.length; i++) {
            ...
            if(entry === "separator")
                ...
                if(entry.hint && entry.hint != "")
                    popup._tooltip = new Tooltips.Tooltip(popup.actor, entry.hint);
                    
                for(let j=0; j<entry.children.length; j++) {
                    let entry2 = entry.children[j];
                    if(entry2 === "separator")
                        popup.menu.addMenuItem(new PopupMenu.PopupSeparatorMenuItem());
                    else if('toggle' in entry2)
                        popup.menu.addMenuItem(new SettingsToggler(popup.menu, entry2));
                    else
                        popup.menu.addMenuItem(new SettingsLauncher(popup.menu, entry2));
                }
