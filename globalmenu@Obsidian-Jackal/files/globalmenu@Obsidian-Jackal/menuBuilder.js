...
function _activateAction(actionName, targetVariant, actionGroups) {
    ...
    // appmenu-gtk-module menu entries often use mnemonic-mangled names
    // (Prefere-nces) while org.gtk.Actions exports Preferences.
    if (group && !has) {
        try {
    ...
    // Prefer direct org.gtk.Actions.Activate — DBusActionGroup activate_action
    // silently no-ops while the group is still empty (async).
    ...
    if (dbusPath) {
        try {
            ...
            if (!has) {
                // Resolve mnemonic-mangled names against live DescribeAll.
                try {
                    let desc = Gio.DBus.session.call_sync(
                        gtk.busName,
                        dbusPath,
                        "org.gtk.Actions",
                        "DescribeAll",
            ...
            // (sava{sv}) — build explicitly for GJS
 ...
 * Activates unity submenu-action (e.g. Recents-0) so appmenu-gtk-module
 * exports nested groups, then refills on items-changed / section updates.
 */
...
function fillPopupFromModel(menu, model, actionGroups) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        let targetVar = null;
        try {
            // Never pass null as VariantType — some GJS builds abort the caller.
        ...
        // Skip action-missing only when we have no D-Bus Activate fallback.
        // DBusActionGroup populates async (~250ms); skipping here empties Transmission submenus.
        if (hiddenWhen === "action-missing" && actionName) {
            let dot = actionName.indexOf(".");
            let ns = dot >= 0 ? actionName.substring(0, dot) : null;
            let an = dot >= 0 ? actionName.substring(dot + 1) : actionName;
            let group = (ns && actionGroups[ns]) || actionGroups["dbusmenu"];
            let gtk = actionGroups && actionGroups._gtk;
            let canDbus = gtk && gtk.busName && (
                (ns === "win" && gtk.winPath) ||
                (ns === "app" && gtk.appPath) ||
                (ns === "unity" && gtk.unityPath)
            );
            if (!canDbus && group && !group.has_action(an) && !group.has_action(actionName))
                continue;
        }
 ...
 * @param {any} parentActor
 * @param {any} menuManager
...
function buildPanelMenus(parentActor, menuManager, orientation, model, actionGroups) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        if (submenu) {
            ...
            // Leaf top-level — rare; treat as single action item
            let actionVar = _getAttr(model, i, Gio.MENU_ATTRIBUTE_ACTION, "s");
            let actionName = _variantToString(actionVar);
...
function _dbusProp(props, key, fallback) {
    if (!props || props[key] === undefined || props[key] === null)
        return fallback;
    let v = props[key];
    // deep_unpack may leave Variants for some peers
    try {
            ...
            if (kids && kids.length)
                ...
                subItem.menu.connect("open-state-changed", (m, open) => {
                    if (!open)
                        return;
                    m.removeAll();
                    let layout = menuSource.fetchDbusMenuLayout(proxy, id, -1);
...
/**
 * Build panel menus from com.canonical.dbusmenu GetLayout (no GMenuModel).
 * @param {any} parentActor
 * @param {any} menuManager
 * @param {number} orientation
 * @param {any} proxy
 * @returns {{actor: any, menu: any, destroy: Function}[]}
 */
function buildPanelFromDbusmenu(parentActor, menuManager, orientation, proxy) {
    let entries = [];
    if (!proxy)
        return entries;

    let layout = menuSource.fetchDbusMenuLayout(proxy, 0, 1);
    ...
    for (let i = 0; i < layout.children.length; i++) {
        ...
        menu.connect("open-state-changed", (m, open) => {
            if (!open)
                return;
            m.removeAll();
            let sub = menuSource.fetchDbusMenuLayout(proxy, itemId, -1);
            if (sub && sub.children)
...
module.exports = {
    fillPopupFromModel,
    buildPanelMenus,
    fillPopupFromDbusChildren,
    buildPanelFromDbusmenu
};
