...
class CinnamenuApplet extends TextIconApplet {
    constructor(metadata, orientation, panel_height, instance_id) {
        ...
        this.menu.setCustomStyleClass('menu-background2 cinnamenu');//starkmenu-background');
        }
        ...
        { key: 'searchbox-placement',         value: 'searchboxPlacement',      cb: refreshDisplay },
        { key: 'searchbox-size',              value: 'searchboxSize',           cb: refreshDisplay },
        ...
        { key: 'show-sidebar-labels',         value: 'showSidebarLabels',       cb: refreshDisplay },
        ...
        { key: 'show-logoutlockscreen',       value: 'showlogoutlockscreen',    cb: refreshDisplay },
        ...
        { key: 'menu-spacing',                value: 'menuSpacing',             cb: this._applyMenuStyle },
        ...
        { key: 'use-user-avatar',             value: 'useUserAvatar',           cb: refreshDisplay },
        { key: 'user-avatar-size',            value: 'userAvatarSize',          cb: refreshDisplay },
...
//----------------TextIconApplet callbacks----------------
    ...
    _applyMenuStyle() {
        if (this.settings.menuSpacing) {
            this.menu.setCustomStyleClass('menu-background cinnamenu');
        } else {
            this.menu.setCustomStyleClass('menu-background2 cinnamenu');
        }
    }
...
//==================================================================
    ...
    _onOpenStateToggled(menu, open) {
        if (global.settings.get_boolean('panel-edit-mode')) {
            ...
            if (!this._firstOpenHandled) {
                this.display.updateMenuSize();
                this.display.sidebar.populate();
                this._firstOpenHandled = true;
            }
        ...
        if (open) {
            ...
            if (!this.settings.showCategories) {
              this.currentCategory = 'mixed';
            } else { this.currentCategory = 'all'; }
            }
            // Always open on the complete application grid. In compact mode,
            // the Back control remains available to show Favorites and Recents.
            const openOnCategory = 'all';
    ...
    log("ACTIVE CATEGORY:", categoryId);
        ...
        case 'mixed': {
            const maxItems = this.getNumberOfItemsToFitColumns(6);
            const maxRecentApps = this.getNumberOfItemsToFitColumns(8);

            this.display.appsView.populate_init();
            this.display.appsView.populate_add(this.listFavoriteApps(), _('Favorites'));  // Favorites ALWAYS shown
            // this.display.appsView.addSpacer(1); // 8px spacer

            // Other recents
            const recents = [
                ...this.listRecentByType('documents', maxItems),
                ...this.listRecentByType('video', maxItems),
                ...this.listRecentByType('image', maxItems),
                ...this.listRecentByType('audio', maxItems)
            ];

            // Recent apps
            const recentApps = this.listRecent_apps(maxRecentApps);
            if (recentApps.length + recents.length > 0) {
                this.display.appsView.populate_add(recentApps, _('Recent'));
            }

            const totalItems = recentApps.length + recents.length;

            if (recents.length > 0) {
            this.display.appsView.addSpacer(0); // Add spacer 
                this.display.appsView.populate_add(recents);
            }

            if (totalItems > 0) {
            this.display.appsView.populate_add(this.getClearRecentsButton());
            } else {
                this.display.appsView.populate_add([], _('No recent Items'));
            }
        ...
        default:
            if (categoryId.startsWith('mixed')) {

            const maxItems = this.getNumberOfItemsToFitColumns(6);
            const maxRecentApps = this.getNumberOfItemsToFitColumns(8);
            this.display.appsView.populate_init();

            this.display.appsView.populate_add(this.listFavoriteApps(),_('Favorites'));
            this.display.appsView.populate_add([], _(' '));
            ...
            this.display.appsView.populate_add([], _('Recent'));}
            ...
            const totalItems = recents.length;
            if (totalItems > 0) {
            this.display.appsView.populate_add(recents,_(' '));
                this.display.appsView.populate_add(this.getClearRecentsButton());
            }
            this.display.appsView.populate_finish();
            if (totalItems == 0) {
                ...
                } else if(categoryId.startsWith('emoji:')) {
                ...
                this.display.appsView.populate_init();
                this.display.appsView.populate_add(this.apps.listApplications('allApps'),_("Applications"));
                this.display.appsView.populate_add(
...
//==============search==============
    ...
    _doSearch(pattern_raw, thisSearchId) {
        ...
        const mathNames = /Math\.(?:E|LN10|LN2|LOG10E|LOG2E|PI|SQRT1_2|SQRT2|abs|acos|acosh|asin|asinh|atan|atanh|cbrt|ceil|clz32|cos|cosh|exp|expm1|floor|fround|hypot|imul|log|log10|log1p|log2|max|min|pow|random|round|sign|sin|sinh|sqrt|tan|tanh|trunc)/g;
        const safeExpression = exp.replace(mathNames, '');
        const containsOnlyMathSyntax = /^[0-9eE+\-*/%().,\s<>=!&|?:]+$/.test(safeExpression);

        try {
            if (containsOnlyMathSyntax) {
                ans = eval(exp);
            }
        } catch(e) {
            const r = /[\(\)\+=/\*\.;,]/
        ...
        if ((typeof ans === 'number' || typeof ans === 'boolean' || typeof ans === 'bigint')
            ...
            otherResults.push({
                            ...
                            deleteAfterUse: true,
                            icon: new St.Icon({ icon_name: 'accessories-calculator-symbolic',
        ...
        if (this.settings.webSearchOption != 0 && !PREFIX_USED) {//0==none
            ...
            otherResults.push({
                        ...
                        description: '',
                        deleteAfterUse: true,
                        icon: new St.Icon({ icon_name: 'system-search-symbolic',
                                            icon_type: St.IconType.SYMBOLIC,
                                            icon_size: this.getAppIconSize()}),
