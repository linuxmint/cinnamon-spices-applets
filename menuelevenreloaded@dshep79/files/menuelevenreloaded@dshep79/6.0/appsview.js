...
class AppButton {
    constructor(appThis, app) {
        ...
        if (this.app.isClearRecentsButton && this.icon.set_icon_size) {
        ...
        if (this.app.isClearRecentsButton) {
            this.label.style = 'font-size: 85%; font-weight: bold;';
        }
    ...
    activateAsRoot() {
        if (this.app.isApplication) {
            ...
            this.appThis.recentApps.add(this.app.id);
            Util.spawn(['gksu', this.app.get_app_info().get_executable()]);
            this.appThis.menu.close();
        } 
    }
