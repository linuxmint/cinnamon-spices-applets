...
const SearchboxPlacement = Object.freeze({TOP: 0, BOTTOM: 1});
...
class Display {
    constructor (appThis) {
        ...
        const searchboxPlacement = this.appThis.settings.searchboxPlacement;
        ...
        const sidebarwidth =  ((this.appThis.settings.customMenuWidth - 
        (this.appThis.settings.customMenuWidth * (this.appThis.settings.searchboxSize / 100)))/2); 

        //==================== Create spacers ===================
        this.leftSpacer = new St.Widget({ width: sidebarwidth, x_expand: false });
        this.rightSpacer = new St.Widget({ width: sidebarwidth, x_expand: false });

        //================= Place the Searchbox =================        
        if (searchboxPlacement === SearchboxPlacement.TOP) {
          this.SearchTopBox = new St.BoxLayout({ style: 'height: 50px;', x_expand: true, y_expand: false });
          this.SearchBottomBox = new St.BoxLayout({ style: 'height: 20px;', x_expand: true, y_expand: false });

          // === add in order: spacer → search → spacer ===
          this.SearchTopBox.add(this.leftSpacer, { expand: false, x_fill: false, y_fill: false });
          this.SearchTopBox.add(this.searchView.searchBox, { expand: true, x_fill: true, y_fill: false, 
                                            x_align: St.Align.FILL, y_align: St.Align.MIDDLE });
          this.SearchTopBox.add(this.rightSpacer, { expand: false, x_fill: false, y_fill: false });

        } else {
          this.SearchTopBox = new St.BoxLayout({ style: 'height: 8px;', x_expand: true, y_expand: false });

          this.SearchBottomBox = new St.BoxLayout({ style: 'height: 50px;', x_expand: true, y_expand: false });


          this.SearchBottomBox.add(this.leftSpacer, { expand: false, x_fill: false, y_fill: false });
          this.SearchBottomBox.add(this.searchView.searchBox, { expand: true, x_fill: true, y_fill: false, 
                                            x_align: St.Align.FILL, y_align: St.Align.MIDDLE });
          this.SearchBottomBox.add(this.rightSpacer, { expand: false, x_fill: false, y_fill: false });
        ...
        if (sidebarPlacement === SidebarPlacement.TOP && this.appThis.settings.showSidebar || 
            sidebarPlacement === SidebarPlacement.BOTTOM  && this.appThis.settings.showSidebar) {
        ...
        // ===== Add Spacer to centralise Apps Icons =====
        this.AppsSpacer = new St.Widget({ width: this.appThis.settings.showCategories ? 0 : 32, 
                                          height: 0, x_expand: false, y_expand: false });

        //=================middlePane======================
        this.appsView = new AppsView(this.appThis);
        this.categoriesView = new CategoriesView(this.appThis);
        this._categoriesFixedWidth = this.categoriesView.groupCategoriesWorkspacesScrollBox.width;
        ...
        // === ADD AppsSpacer(left of middle content) ===
        this.middlePane.add(this.AppsSpacer, { expand: false, x_fill: false, y_fill: false,
        ...
        if (sidebarPlacement === SidebarPlacement.RIGHT && this.appThis.settings.showSidebar ) {
            this.middlePane.add(this.sidebar.sidebarOuterBox, { expand: false, x_fill: false, y_fill: false,
                                                    x_align: St.Align.START, y_align: St.Align.MIDDLE });
        }
        ...
        this.mainBox.add(this.SearchTopBox);    // Add search top search box first (top of menu)

        if (sidebarPlacement === SidebarPlacement.TOP || !this.appThis.settings.showSidebar) {
            this.mainBox.add(this.SearchBottomBox);   // add bottom search box
            this.mainBox.add(this.bottomPane);
        }
        this.mainBox.add_actor(this.middlePane);
        // Add bottom search box BEFORE bottomPane (so it's above it)
        if (sidebarPlacement !== SidebarPlacement.TOP && this.appThis.settings.showSidebar) {
            this.mainBox.add(this.SearchBottomBox);
            this.mainBox.add(this.bottomPane);
        }
    ...
    updateMenuSize(newWidth, newHeight) {
        ...
        const bottomPaneHeight = this._bottomPaneHeight || this.bottomPane.height;
        this._bottomPaneHeight = bottomPaneHeight;

        let appsHeight = newHeight - bottomPaneHeight;
        ...
        //------------width-------------
        let leftSideWidth = this.appThis.settings.showCategories
          ? this._categoriesFixedWidth: 0; // hide categories, no extra width
...
class SearchView {
    constructor(appThis) {
        ...
        this.searchEntry = new St.Entry({ name: 'menu-search-entry', hint_text: _("Type to search..."), 
