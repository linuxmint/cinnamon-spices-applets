...
Time_change_listener::Time_change_listener(std::function<void()> callback) :
    fd{new_timerfd()},
    callback{callback},
    listener_thread{&This::listener_thread_function, this}
{}

void Time_change_listener::listener_thread_function() const {
    do {
        ...
        if (-1 == ::read(this->fd, &dull_buffer, sizeof(dull_buffer))) {
            if (errno == ECANCELED) { // system time changed
                ...
                std::cerr << "Time_change_listener::listener_thread_function: "
                             "read failed: " << std::strerror(errno)
                          << std::endl;
                return;
            }
        }
    } while (this->should_listener_thread_run);
}

static constexpr struct itimerspec SETTIME_NEVER = {{0, 0}, {0, 0}};

void Time_change_listener::enable() const {
    static constexpr auto FLAGS = TFD_TIMER_ABSTIME | TFD_TIMER_CANCEL_ON_SET;
...
Time_change_listener::~Time_change_listener() {
    this->should_listener_thread_run = false;

    static constexpr struct itimerspec VIRTUALLY_NOW = {{0, 0}, {0, 1}}; // "Once, in 1 ns"

    if (-1 == ::timerfd_settime(this->fd, 0, &VIRTUALLY_NOW, nullptr)) // Unblocks the blocked `read` call in the `listener_thread_function` method
