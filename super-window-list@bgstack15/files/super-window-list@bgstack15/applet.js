...
MyApplet.prototype = {
    ...
    _init: function(orientation, panel_height, instance_id) {
        ...
        this.settings = new Settings.AppletSettings(this, "super-window-list@bgstack15", this.instance_id);

	this.settings.bind("show-windows-all-workspaces", "showWindowsFromAllWorkspaces");
    ...
    _refreshItem: function(window) {
        // this lets workspace 1 show workspace 1 windows
        window.actor.visible =
            ...
            this.showWindowsFromAllWorkspaces;
    ...
    _updateWatchedMonitors: function() {
        ...
        // Now track the windows in our favorite monitors FINDTHIS
        let windows = global.display.list_windows(0);
	if (this.showWindowsFromAllWorkspaces === true) {
		// reference: windows-quick-list@cinnamon.org
	...
	// debug
	//global.log("Evaluating these windows:");

        for (let window of windows) {
	    //global.log(window.title);
            if (this._shouldAdd(window)) {
	        //global.log(window.title.concat(" made it!"));
	...
	// FINDTHIS the fix for all workspaces is here
        if (alert) {
