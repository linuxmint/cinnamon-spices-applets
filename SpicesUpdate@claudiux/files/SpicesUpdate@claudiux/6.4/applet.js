...
class SpicesUpdate extends IconApplet {
  ...
  _loop_refresh_cache() {
    ...
    for (let t of TYPES) {
      let jsonFile = file_new_for_path(CACHE_MAP[t]);
  ...
  get_SU_settings() {
    ...
    this.settings.bind("dont_check_dependencies", "dont_check_dependencies");
  ...
  are_dependencies_installed() {
    if (this.dont_check_dependencies === true) return true;
    ...
    if (!_fonts_installed) {
      ...
      _GentoWitnessFile = null;
      _openSUSEWitnessFile = null;
      _ArchlinuxWitnessFile = null;
    } else {
      this.dont_check_dependencies = true;
    }
    this.fonts_installed = _fonts_installed;
    _fonts_installed = null;
  ...
  check_dependencies() {
    if (!this.dependenciesMet && this.are_dependencies_installed()) {
      ...
      free(_is_xdg_open_present);
      free(_isFedora);
      _DebianWitnessFile = null;
      _openSUSEWitnessFile = null;
      _ArchlinuxWitnessFile = null;
      _GentoWitnessFile = null;
    }
  ...
  _load_cache(type) {
    ...
    if (jsonFile.query_exists(null)) {
      ...
      let contents = file_get_contents(jsonFileName)[1];
      this.cache[type] = to_string(contents);
      free(contents);
    } else {
      this.cache[type] = "{}";
    }
