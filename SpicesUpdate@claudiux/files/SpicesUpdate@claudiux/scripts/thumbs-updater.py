...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Cinnamon Spices Thumbs Updater',
                                         epilog="spice-type should be 'applet', 'desklet', 'extension' or 'theme'.")
    parser.add_argument("--update-all", dest="update_all_thumbs", action="store_const", const=True,
                        help="Update all spices thumbs to their latest versions.")

    choices = ["applet", "desklet", "extension", "theme"]

    ex_group = parser.add_mutually_exclusive_group()
    ex_group.add_argument("--update", dest="update_thumbs_of_type", action="store", choices=choices, metavar="<spice-type>",
    ...
    def update_thumbs_of_type():
      h = Harvester(args.update_thumbs_of_type)
      h.refresh()
