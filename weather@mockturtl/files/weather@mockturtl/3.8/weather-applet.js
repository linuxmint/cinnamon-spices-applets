...
function extractISODuration(match) {
  ...
  var maybeNegate = (num, force = false) => num !== undefined && (force || num && hasNegativePrefix) ? -num : num;
  return [{
...
class Interval {
  ...
  static merge(intervals) {
    var _intervals$sort$reduc = intervals.sort((a, b) => a.s - b.s).reduce(([sofar, current], item) => {
        if (!current) {
          return [sofar, item];
...
function explainFromTokens(locale, input, format) {
  ...
  if (disqualifyingUnit) {
    ...
    var _buildRegex = buildRegex(units),
      ...
      matches = _match2[1],
      _ref = matches ? dateTimeFromMatches(matches) : [null, null, undefined],
      _ref2 = tokenParser_slicedToArray(_ref, 3),
      result = _ref2[0],
      zone = _ref2[1],
      specificOffset = _ref2[2];
...
class Soup3 {
    ...
    async Send(url, options = {}) {
        const { params, headers, method = "GET", cancellable, noEncode = false, logUrl } = options;
        await this.defaultUserAgentReady;
        if (cancellable === null || cancellable === void 0 ? void 0 : cancellable.is_cancelled()) {
            return null;
        }
        const urls = BuildRequestUrls(url, params !== null && params !== void 0 ? params : undefined, logUrl, !noEncode);
        const query = urls.requestUrl;
        const safeQuery = urls.logUrl;
        logger_Logger.Debug("URL called: " + safeQuery);
...
class Soup2 {
    ...
    async Send(url, options = {}) {
        const { params, headers, method = "GET", cancellable, logUrl } = options;
        await this.defaultUserAgentReady;
        if (cancellable === null || cancellable === void 0 ? void 0 : cancellable.is_cancelled()) {
            return null;
        }
        const urls = BuildRequestUrls(url, params !== null && params !== void 0 ? params : undefined, logUrl);
        const query = urls.requestUrl;
        const safeQuery = urls.logUrl;
        logger_Logger.Debug("URL called: " + safeQuery);
        const data = await new Promise((resolve) => {
            ...
            if (message == null) {
                ...
                logger_Logger.Debug("Sending http request to " + safeQuery);
                this._httpSession.send_async(message, finalCancellable, async (session, result) => {
                    ...
                    let res = null;
                    try {
                        logger_Logger.Debug("Reading reply from " + safeQuery);
                        const stream = this._httpSession.send_finish(result);
                        logger_Logger.Debug("Reply received from " + safeQuery + " with status code " + message.status_code + " and reason: " + message.reason_phrase);
...
class OpenWeatherMapOneCall {
    ...
    async GetWeather(loc, cancellable, config, customConfig) {
...
;// CONCATENATED MODULE: ./src/3_8/lib/unitConversions.ts
...
function PascalsToHectopascals(pressure) {
...
class Weatherbit {
    ...
    async GetWeather(loc, cancellable, config, options) {
        ...
        if (this.hourlyAccess)
            hourlyPromise = this.GetHourlyData(this.hourly_url, loc, cancellable, config, options);
    ...
    async GetHourlyData(baseUrl, loc, cancellable, config, options) {
        const query = this.ConstructQuery(loc, config, options);
        if (query == null)
            return null;
        const json = await HttpLib.Instance.LoadJsonSimple({
    ...
    ConstructQuery(loc, config, options) {
        const result = {
            key: options.apiKey,
            lat: loc.lat,
            lon: loc.lon,
    ...
    async GetWeather(loc, cancellable, config, options) {
        ...
        if (this.locationCache[locationID] != null) {
            ...
            location = await HttpLib.Instance.LoadJsonSimple({
                ...
                cancellable,
                params: { q: locationID, details: true, language: locale, apikey: options.apiKey },
        ...
        const [current, forecast, hourly] = await Promise.all([
            HttpLib.Instance.LoadJsonAsync({
                ...
                cancellable,
                params: { apikey: options.apiKey, details: true, language: locale, },
                HandleError: this.HandleErrors
            }),
            HttpLib.Instance.LoadJsonAsync({
                url: this.dailyForecastUrl + location.Key,
                cancellable,
                params: { apikey: options.apiKey, details: true, metric: true, language: locale, },
                HandleError: this.HandleErrors
            }),
            HttpLib.Instance.LoadJsonAsync({
...
class WeatherUnderground {
    constructor() {
        ...
        this.locationCache = {};
        this.GetWeather = async (loc, cancellable, config, options) => {
            var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k, _l, _m;
            const locString = `${loc.lat},${loc.lon}`;
            const location = (_a = this.locationCache[locString]) !== null && _a !== void 0 ? _a : (await this.GetNearbyStations(loc, cancellable, options));
            if (location == null) {
                return null;
            }
            ...
            if (forecast == null)
                return null;
            const observation = await this.GetObservations(location, forecast, loc, cancellable, config, options);
            return {
        ...
        this.GetNearbyStations = async (loc, cancellable, options) => {
            var _a;
            const result = [];
        ...
        this.GetObservations = async (stations, forecast, loc, cancellable, config, options) => {
            var _a;
            const observationData = (await Promise.all(stations.map(v => this.GetObservation(v.stationId, cancellable, options)))).filter(v => v != null);
        ...
        this.GetObservation = async (stationID, cancellable, options) => {
            var _a;
...
class MetUk {
    constructor() {
        ...
        this.observationBaseUrl = "https://data.hub.api.metoffice.gov.uk/observation-land/1";
        this.observationUrl = `${this.observationBaseUrl}/nearest`;
        this.hourlyForecastUrl = `${this.baseUrl}/point/hourly`;
        this.forecastUrl = `${this.baseUrl}/point/daily`;
    }
    async GetWeather(newLocation, cancellable, config, options) {
        var _a, _b, _c, _d, _e, _f, _g, _h;
        ...
        if (!observationStationRes.Success) {
            logger_Logger.Error(`MetUK: Failed to get observation station data: ${JSON.stringify(observationStationRes.ErrorData)}`);
            return null;
        }
        ...
        if (!observationStation) {
            logger_Logger.Error(`MetUK: No observation station data found for location ${newLocation.lat}, ${newLocation.lon}`);
            return null;
        }
        const times = (0,suncalc.getTimes)(new Date(), newLocation.lat, newLocation.lon, hourlyRes.Data.features[0].geometry.coordinates[2]);
        const observationRes = await HttpLib.Instance.LoadJsonAsync({
        ...
        if (!observationRes.Success) {
            logger_Logger.Error(`MetUK: Failed to get observation data: ${JSON.stringify(observationRes.ErrorData)}`);
            return null;
        }
        const now = DateTime.now();
        ...
        if (hourlyForecasts.timeSeries.length === 0) {
            logger_Logger.Error(`MetUK: No relevant hourly forecast data found for location ${newLocation.lat}, ${newLocation.lon}`);
            return null;
        }
        ...
        return {
            condition: this.ResolveCondition((_c = observationLast === null || observationLast === void 0 ? void 0 : observationLast.weather_code) !== null && _c !== void 0 ? _c : hourlyForecastFirst.significantWeatherCode),
            coord: {
                lat: newLocation.lat,
                lon: newLocation.lon,
            },
            ...
            wind: {
                speed: (_d = observationLast === null || observationLast === void 0 ? void 0 : observationLast.wind_speed) !== null && _d !== void 0 ? _d : hourlyForecastFirst.windSpeed10m,
                degree: (observationLast === null || observationLast === void 0 ? void 0 : observationLast.wind_direction) ? CompassToDeg(observationLast.wind_direction) : hourlyForecastFirst.windDirectionFrom10m,
            },
            humidity: (_e = observationLast === null || observationLast === void 0 ? void 0 : observationLast.humidity) !== null && _e !== void 0 ? _e : hourlyForecastFirst.screenRelativeHumidity,
            pressure: (_f = observationLast === null || observationLast === void 0 ? void 0 : observationLast.mslp) !== null && _f !== void 0 ? _f : hourlyForecastFirst.mslp,
            dewPoint: null,
            ...
            location: {
                ...
                country: (_g = observationStation.country) !== null && _g !== void 0 ? _g : undefined,
            },
            temperature: CelsiusToKelvin((_h = observationLast === null || observationLast === void 0 ? void 0 : observationLast.temperature) !== null && _h !== void 0 ? _h : hourlyForecastFirst.screenTemperature),
    ...
    GetDailyForecasts(payload, timezone) {
        var _a;
        const result = [];
        for (const item of payload.timeSeries) {
            result.push({
...
;// CONCATENATED MODULE: ./src/3_8/config-redaction.ts
...
;// CONCATENATED MODULE: ./src/3_8/providers/qweather/config.ts
const DEDICATED_API_HOST = /^(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+qweatherapi\.com$/;
function QWeatherLocation(location) {
    return `${location.lon.toFixed(2)},${location.lat.toFixed(2)}`;
}
function QWeatherAlertPath(location) {
...
function QWeatherLanguage(locale, translateCondition) {
    if (!translateCondition)
        return "en";
    const normalized = locale === null || locale === void 0 ? void 0 : locale.toLowerCase();
    if (!normalized)
        return "en";
...
;// CONCATENATED MODULE: ./src/3_8/lib/precipitation.ts
function FindPrecipitationWindow(values, stepMinutes) {
    let start = -1;
...
;// CONCATENATED MODULE: ./src/3_8/providers/qweather/condition.ts
function ConditionWithIcons(main, icons, customIcon) {
    return {
        main,
        description: main,
        icons,
...
function QWeatherCondition(icon, text, translate) {
...
;// CONCATENATED MODULE: ./src/3_8/providers/qweather/parser.ts
...
function QWeatherHourlyToData(hour, location, translate) {
    const chance = FiniteNumber(hour.pop);
    const volume = FiniteNumber(hour.precip);
    const hasPrecipitation = (chance !== null && chance !== void 0 ? chance : 0) > 0 || (volume !== null && volume !== void 0 ? volume : 0) > 0;
    return {
        ...
        precipitation: hasPrecipitation ? {
            ...
            chance: chance !== null && chance !== void 0 ? chance : undefined,
            volume: volume !== null && volume !== void 0 ? volume : undefined,
        } : undefined,
    };
}
function QWeatherAlertLevel(severity) {
    if (severity === "minor" || severity === "moderate" || severity === "severe" || severity === "extreme")
        return severity;
    return "unknown";
}
function QWeatherAlertToData(alert, attributions) {
    var _a;
    return {
        sender_name: (_a = alert.senderName) !== null && _a !== void 0 ? _a : "QWeather",
...
function QWeatherResponseToData(payloads, location, translate) {
    var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k;
    const { current } = payloads;
    if (current.code !== "200" || !current.now)
        return null;
    const daily = ((_a = payloads.daily) === null || _a === void 0 ? void 0 : _a.code) === "200" ? (_b = payloads.daily.daily) !== null && _b !== void 0 ? _b : [] : [];
    const hourly = ((_c = payloads.hourly) === null || _c === void 0 ? void 0 : _c.code) === "200" ? (_d = payloads.hourly.hourly) !== null && _d !== void 0 ? _d : [] : [];
    const firstDaily = daily[0];
    const minutely = ((_e = payloads.minutely) === null || _e === void 0 ? void 0 : _e.code) === "200" ? payloads.minutely.minutely : undefined;
    const attributions = (_h = (_g = (_f = payloads.alerts) === null || _f === void 0 ? void 0 : _f.metadata) === null || _g === void 0 ? void 0 : _g.attributions) !== null && _h !== void 0 ? _h : [];
    const alerts = (_k = (_j = payloads.alerts) === null || _j === void 0 ? void 0 : _j.alerts) === null || _k === void 0 ? void 0 : _k.map(alert => QWeatherAlertToData(alert, attributions));
    const windSpeed = FiniteNumber(current.now.windSpeed);
    return Object.assign(Object.assign({ date: QWeatherTime(current.now.obsTime, location), coord: { lat: location.lat, lon: location.lon }, location: {
            city: location.city,
            country: location.country,
            timeZone: location.timeZone,
            url: current.fxLink,
        }, sunrise: firstDaily ? LocalTime(firstDaily.fxDate, firstDaily.sunrise, location) : null, sunset: firstDaily ? LocalTime(firstDaily.fxDate, firstDaily.sunset, location) : null, wind: {
            speed: windSpeed == null ? null : KPHtoMPS(windSpeed),
            degree: FiniteNumber(current.now.wind360),
        }, uvIndex: FiniteNumber(firstDaily === null || firstDaily === void 0 ? void 0 : firstDaily.uvIndex), temperature: CelsiusToKelvin(FiniteNumber(current.now.temp)), pressure: FiniteNumber(current.now.pressure), humidity: FiniteNumber(current.now.humidity), dewPoint: CelsiusToKelvin(FiniteNumber(current.now.dew)), condition: QWeatherCondition(current.now.icon, current.now.text, translate), forecasts: daily.map(day => ({
        ...
        })), hourlyForecasts: hourly.map(hour => QWeatherHourlyToData(hour, location, translate)), extra_field: (() => {
            var _a;
            const feelsLike = CelsiusToKelvin(FiniteNumber((_a = current.now) === null || _a === void 0 ? void 0 : _a.feelsLike));
...
;// CONCATENATED MODULE: ./src/3_8/providers/qweather/provider.ts
...
class QWeather {
    constructor() {
        this.prettyName = _("QWeather");
        this.name = Services.QWeather;
        this.maxForecastSupport = 7;
        this.maxHourlyForecastSupport = 24;
        this.website = "https://www.qweather.com/";
        this.needsApiKey = true;
        this.supportHourlyPrecipChance = true;
        this.supportHourlyPrecipVolume = true;
        this.locationType = "coordinates";
    ...
    async GetWeather(loc, cancellable, config, options) {
        ...
        const currentPromise = this.Load(`${baseUrl}/v7/weather/now`, params, cancellable, options, false);
        const hourlyPromise = this.Load(`${baseUrl}/v7/weather/24h`, params, cancellable, options, true);
        const dailyPromise = this.Load(`${baseUrl}/v7/weather/7d`, params, cancellable, options, true);
        const minutelyPromise = config._immediatePrecip
            ? this.Load(`${baseUrl}/v7/minutely/5m`, params, cancellable, options, true)
            : Promise.resolve(null);
        const alertsPromise = config._showAlerts
            ? this.Load(`${baseUrl}${QWeatherAlertPath(loc)}`, {
                localTime: true,
    ...
    async Load(url, params, cancellable, options, optional) {
        const response = await HttpLib.Instance.LoadJsonSimple({
            url,
            params,
            cancellable,
    ...
    HandleHttpError(error, optional) {
...
class Config {
    ...
    constructor(instanceID) {
        ...
        this.OpenWeatherMapOneCallApiKeyChanged = new Event();
        this.MetofficeForecastApiKeyChanged = new Event();
        this.MetofficeObservationsApiKeyChanged = new Event();
        this.PirateWeatherApiKeyChanged = new Event();
        this.VisualCrossingApiKeyChanged = new Event();
        this.WeatherbitApiKeyChanged = new Event();
        this.TomorrowIOApiKeyChanged = new Event();
        this.AccuWeatherApiKeyChanged = new Event();
        this.WeatherUndergroundApiKeyChanged = new Event();
        this.QWeatherApiHostChanged = new Event();
        this.QWeatherApiKeyChanged = new Event();
    ...
    GetServiceConfig(service) {
        ...
        let newApiKeyField = null;
        switch (service) {
            case Services.OpenWeatherMap_OneCall:
                newApiKeyField = Keys.OPENWEATHERMAP_ONECALL_API_KEY.key;
                break;
            case Services.PirateWeather:
                newApiKeyField = Keys.PIRATEWEATHER_APIKEY.key;
                break;
            case Services.VisualCrossing:
                newApiKeyField = Keys.VISUALCROSSING_APIKEY.key;
                break;
            case Services.Weatherbit:
                newApiKeyField = Keys.WEATHERBIT_APIKEY.key;
                break;
            case Services.Tomorrow_IO:
                newApiKeyField = Keys.TOMORROW_IO_APIKEY.key;
                break;
            case Services.AccuWeather:
                newApiKeyField = Keys.ACCUWEATHER_APIKEY.key;
                break;
            case Services.WeatherUnderground:
                newApiKeyField = Keys.WEATHER_UNDERGROUND_APIKEY.key;
                break;
        }
        if (newApiKeyField != null && !this[`_${newApiKeyField}`] && !!oldApiKey) {
            ...
            logger_Logger.Info(`Migrating old API key to new field for service ${service}`);
        }
        switch (service) {
            case Services.OpenWeatherMap_OneCall: {
                const result = {
...
const { Tooltip: uiHourlyForecasts_Tooltip } = imports.ui.tooltips;
...
class UIHourlyForecasts {
    ...
    Display(forecasts, config, tz) {
        ...
        for (let index = 0; index < max; index++) {
            ...
            (_a = ui.SummaryTooltip) === null || _a === void 0 ? void 0 : _a.set_text(hour.condition.main);
    ...
    Rebuild(config, textColorStyle, availableHours = null) {
        ...
        for (let index = 0; index < hours; index++) {
            const box = new uiHourlyForecasts_BoxLayout({ vertical: true, style_class: "hourly-box-item", reactive: true });
            ...
            const hourlySet = this.hourlyForecasts[index];
            hourlySet.PrecipVolume.clutter_text.set_line_wrap(true);
            ...
            if ((_b = this.app.Provider) === null || _b === void 0 ? void 0 : _b.supportHourlyPrecipVolume)
                box.add_child(hourlySet.PrecipVolume);
            hourlySet.SummaryTooltip = new uiHourlyForecasts_Tooltip(hourlySet.Icon, _(ELLIPSIS));
