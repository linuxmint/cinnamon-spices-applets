...
function maybeArray(thing: any) {
  return Array.isArray(thing) ? thing : [thing];
}

function bestBy(arr: any[], by: any, compare: any) {
  if (arr.length === 0) {
    return undefined;
  }
...
function integerBetween(thing: any, bottom: any, top: any) {
...
function padStart(input: any, n = 2) {
...
function parseInteger(string: any) {
...
function roundTo(number: any, digits: any, towardZero = false) {
...
function parseZoneInfo(ts: any, offsetFormat: any, locale: any, timeZone: any = null) {
  ...
  if (timeZone) {
    (intlOpts as any).timeZone = timeZone;
  }
...
function normalizeObject(obj: any, normalizer: any, nonUnitKeys: any) {
...
function formatOffset(offset: any, format: any) {
...
function meridiemForDateTime(dt: any) {
  return meridiems[dt.hour < 12 ? 0 : 1];
}

function weekdayForDateTime(dt: any, length: number) {
  return weekdays(length)[dt.weekday - 1];
}

function monthForDateTime(dt: any, length: number) {
  return months(length)[dt.month - 1];
}

function eraForDateTime(dt: any, length: number) {
  return eras(length)[dt.year < 0 ? 0 : 1];
}

function formatRelativeTime(unit: any, count: any, numeric = "always", narrow = false) {
  const units = {
    years: ["year", "yr."],
...
function formatString(knownFormat: any) {
...
function stringifyTokens(splits: any, tokenToString: any) {
  let s = "";
...
class Formatter {
  ...
  static create(locale: any, opts = {}) {
    return new Formatter(locale, opts);
  }

  static parseFormat(fmt: any) {
    let current = null,
      currentFull = "",
      bracketed = false;
    const splits = [];
  ...
  static macroTokenToFormatOpts(token: any) {
    return macroTokenToFormatOpts[token];
  }

  constructor(locale: any, formatOpts: any) {
    this.opts = formatOpts;
    this.loc = locale;
    this.systemLoc = null;
  }

  formatWithSystemDefault(dt: any, opts: any) {
  ...
  formatDateTime(dt: any, opts = {}) {
    const df = this.loc.dtFormatter(dt, Object.assign({}, this.opts, opts));
    return df.format();
  }

  formatDateTimeParts(dt: any, opts = {}) {
    const df = this.loc.dtFormatter(dt, Object.assign({}, this.opts, opts));
    return df.formatToParts();
  }

  resolvedOptions(dt: any, opts = {}) {
    const df = this.loc.dtFormatter(dt, Object.assign({}, this.opts, opts));
  ...
  formatDateTimeFromString(dt: any, fmt: any) {
    const knownEnglish = this.loc.listingMode() === "en",
      useDateTimeFormatter =
        this.loc.outputCalendar && this.loc.outputCalendar !== "gregory" && hasFormatToParts(),
      string = (opts: any, extract: any) => this.loc.extract(dt, opts, extract),
      formatOffset = (opts: any) => {
      ...
      month = (length: any, standalone: any) =>
        knownEnglish
          ? monthForDateTime(dt, length)
          : string(standalone ? { month: length } : { month: length, day: "numeric" }, "month"),
      weekday = (length: any, standalone: any) =>
        knownEnglish
  ...
  formatDurationFromString(dur: any, fmt: any) {
    const tokenToField = (token: any) => {
        switch (token[0]) {
          case "S":
            return "millisecond";
          case "s":
      ...
      tokenToString = (lildur: any) => (token: any) => {
...
class Invalid {
    reason: string;
    explanation: string;
  constructor(reason: string, explanation: string) {
    this.reason = reason;
...
class Zone {
   ...
   * @return {string}
   */
  offsetName(ts: any, opts: any): any {
    throw new ZoneIsAbstractError();
  }
   ...
   * @return {string}
   */
  formatOffset(ts: any, format: any): any {
    throw new ZoneIsAbstractError();
  }
