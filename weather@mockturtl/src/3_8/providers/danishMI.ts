...
export class DanishMI implements WeatherProvider {
	...
	private ParseWeather(observations: DanishObservationPayload[], forecasts: DanishMIPayload): WeatherData {
		...
		result.date = DateTime.fromJSDate(this.DateStringToDate(forecasts.lastupdate), {zone: "UTC"});
		...
		result.sunrise = DateTime.fromJSDate(this.DateStringToDate(forecasts.sunrise), {zone: "UTC"});
		result.sunset = DateTime.fromJSDate(this.DateStringToDate(forecasts.sunset), {zone: "UTC"});
		...
		for (let index = 0; index < forecasts.aggData.length - 1; index++) {
			...
			forecastData.push({
				date: DateTime.fromJSDate(this.DateStringToDate(element.time), {zone: "UTC"}),
