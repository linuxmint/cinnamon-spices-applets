...
export interface WeatherUndergroundOptions {
	apiKey: string;
}

export class WeatherUnderground implements WeatherProvider<Services.WeatherUnderground, WeatherUndergroundOptions> {
	public readonly needsApiKey: boolean = true;
	public readonly prettyName: string = _("Weather Underground");
	public readonly name = Services.WeatherUnderground;
	public readonly maxForecastSupport: number = 6;
	public readonly maxHourlyForecastSupport: number = 0;
	public readonly website: string = "https://www.wunderground.com/";
	...
	public GetWeather = async (loc: LocationData, cancellable: imports.gi.Gio.Cancellable, config: Config, options: WeatherUndergroundOptions): Promise<WeatherData | null> => {
		const locString = `${loc.lat},${loc.lon}`;
		const location = this.locationCache[locString] ?? (await this.GetNearbyStations(loc, cancellable, options));
		if (location == null) {
			// TODO: handle error
			return null;
		}
	...
	private GetNearbyStations = async (loc: LocationData, cancellable: imports.gi.Gio.Cancellable, options: WeatherUndergroundOptions): Promise<NearbyStation[] | null> => {
	...
	private GetObservation = async (stationID: string, cancellable: imports.gi.Gio.Cancellable, options: WeatherUndergroundOptions): Promise<ObservationData | null> => {
