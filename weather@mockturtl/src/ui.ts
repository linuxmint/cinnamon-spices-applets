...
import { WeatherData, WeatherProvider, HourlyForecastData, APIUniqueField } from "./types";
import { shadeHexColor, delay, capitalizeFirstLetter, _, nonempty, AwareDateString, TempToUserConfig, compassDirection, MPStoUserUnits, PressToUserUnits, GetHoursMinutes, GetDayName, MetreToUserUnits, MillimeterToUserUnits, UnitToUnicode } from "./utils";
...
class CurrentWeather extends Bin {
    ...
    constructor(config: Config) {
        super();
        this.config = config;
        this.style_class = STYLE_CURRENT;
    }
    ...
    public SetSunriseAndSunset(sunrise: Date, sunset: Date, tz: string): void {
        let sunriseText = "";
        let sunsetText = "";
        if (sunrise != null && sunset != null && this.config._showSunrise) {
            sunriseText = (GetHoursMinutes(sunrise, this.config.currentLocale, this.config._show24Hours, tz));
            sunsetText = (GetHoursMinutes(sunset, this.config.currentLocale, this.config._show24Hours, tz));
        }
    ...
    public SetAPIUniqueField(extra_field: APIUniqueField) {
        // API Unique display
        ...
        if (!!extra_field) {
            ...
            switch (extra_field.type) {
                case "percent":
                    ...
                    break;
                case "temperature":
                    value = TempToUserConfig(extra_field.value, this.config.TemperatureUnit, this.config._tempRussianStyle) + " " + UnitToUnicode(this.config.TemperatureUnit);
                    break;
                default:
    ...
    public SetWeatherIcon(iconName: string) {
        this._currentWeatherIcon.icon_name = iconName;
    }

    public SetConditionText(condition: string) {
        this._currentWeatherSummary.text = condition;
    }

    public SetTemperature(tempString: string) {
        this._currentWeatherTemperature.text = tempString + " " + UnitToUnicode(this.config.TemperatureUnit);
    }
    ...
    public SetWind(windSpeed: number, windDegree: number) {
        let wind_direction = compassDirection(windDegree);
            this._currentWeatherWind.text =
                (wind_direction != undefined ? _(wind_direction) + " " : "") +
                MPStoUserUnits(windSpeed, this.config.WindSpeedUnit);
            // No need to display unit to Beaufort scale
            if (this.config.WindSpeedUnit != "Beaufort") this._currentWeatherWind.text += " " + _(this.config.WindSpeedUnit);
    }

    public SetPressure(pressure: number) {
        if (pressure != null) {
            this._currentWeatherPressure.text = PressToUserUnits(pressure, this.config._pressureUnit) + ' ' + _(this.config._pressureUnit);
        }
    }
...
export class UI {
	...
	private onLocationStorageChanged(sender: LocationStore, itemCount: number): void {
		...
		if (this.App.config.LocStore.ShouldShowLocationSelectors(this.App.config.CurrentLocation))
			this._currentWeather.ShowLocationSelectors();
		else
			this._currentWeather.HideLocationSelectors();
	}

    /** Creates the skeleton of the popup menu */
    private BuildPopupMenu(): void {
        //  Current Weather
        this._currentWeather = new CurrentWeather(this.App.config);
        //  Daily Weather
    ...
    public Rebuild(config: Config): void {
        this.showLoadingUi();
        this._currentWeather.Rebuild(config, this.GetTextColorStyle());
    ...
    public UpdateIconType(iconType: imports.gi.St.IconType): void {
        ...
        this._currentWeather.ChangeIconType(iconType);
    ...
    public DisplayWeather(weather: WeatherData, config: Config): boolean {
        try {
            ...
            // Weather Condition
            this._currentWeather.SetConditionText(descriptionCondition);
            ...
            if (config._useCustomMenuIcons) {
                this._currentWeather.SetWeatherIcon(weather.condition.customIcon);
                this.UpdateIconType(IconType.SYMBOLIC); // Hard set to symbolic as iconset is symbolic
            }
            else {
                this._currentWeather.SetWeatherIcon(iconName);
            ...
            if (weather.temperature != null) {
                ...
                this._currentWeather.SetTemperature(temp);
            }
            ...
            if (this.App.Orientation != Side.LEFT && this.App.Orientation != Side.RIGHT) {
                ...
                if (config._showTextInPanel) {
                    ...
                    label += (temp + ' ' + UnitToUnicode(config.TemperatureUnit));
                }
            }
            // Vertical panels
            else {
                if (config._showTextInPanel) {
                    ...
                    if (this.App.GetPanelHeight() >= 35) {
                        label += UnitToUnicode(config.TemperatureUnit);
                    }
                }
            }
            ...
            if (nonempty(config._tempTextOverride)) {
                label = config._tempTextOverride
                    .replace("{t}", temp)
                    .replace("{u}", UnitToUnicode(config.TemperatureUnit))
            ...
            this._currentWeather.SetHumidity(weather.humidity);
            this._currentWeather.SetWind(weather.wind.speed, weather.wind.degree);

            this._currentWeather.SetPressure(weather.pressure);
            ...
            this._currentWeather.SetSunriseAndSunset(weather.sunrise, weather.sunset, weather.location.timeZone);
            return true;
        } catch (e) {
