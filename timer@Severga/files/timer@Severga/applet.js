...
TimerApplet.prototype = {
	...
	_init: function(metadata, orientation, panel_height, instance_id) {
		...
		this.set_applet_tooltip("Timer: inactive");
		...
		this.periodMenuItem = new PopupMenu.PopupMenuItem("In:  00:00:00", {reactive: false});
		...
		this.timeMenuItem = new PopupMenu.PopupMenuItem("At:  --:--:--", {reactive: false});
		...
		this.turnOffSwitchMenuItem = new PopupMenu.PopupSwitchMenuItem("Turn off on timer");
		...
		this.quickMenuItem = new PopupMenu.PopupSubMenuMenuItem("Quick setup");
		let tip1 = new PopupMenu.PopupMenuItem("        Tip: set slider to zero to deactivate timer", {reactive: false});
		...
		this.preciseMenuItem = new PopupMenu.PopupSubMenuMenuItem("Precise setup");
		...
		this.preciseMenuItem.menu.addAction("Set time (today)", function () {
			...
			if (this.period > 0) {
				...
				this.periodM = 0;
				this.periodS = 0;
				this.periodMenuItem.setLabel("In:  00:00:00");
				this.timeMenuItem.setLabel("At:  --:--:--");
			}
		}.bind(this));
		this.preciseMenuItem.menu.addAction("Set time (tomorrow)", function () {
			this._remove_timeout();
			this._remove_flashTimeout();
			this.actor.set_style("");
			this.set_applet_label("");
		...
		this.preciseMenuItem.menu.addAction("Set period", function () {
			this._remove_timeout();
			this._remove_flashTimeout();
			this.actor.set_style("");
			this.set_applet_label("");
			...
			else this.timeMenuItem.setLabel("At:  --:--:--");
		}.bind(this));
		this.preciseMenuItem.menu.addAction("Add preset", function () {
		...
		this.presetsMenuItem = new PopupMenu.PopupSubMenuMenuItem("Presets");
	...
	_createPresetsMenuItem: function () {
		...
		let tip2 = new PopupMenu.PopupMenuItem("        Tip: use 'Precise setup' to add preset and  [ × ]  button to remove preset", {reactive: false});
		...
		for (let i = 0; i < this.preset.length; i++) {
			...
			this.presetMenuItem[i].connect("activate", function () {
				...
				this.applet.set_applet_tooltip("Timer: inactive");
				...
				else this.applet.timeMenuItem.setLabel("At:  --:--:--");
			}.bind({applet: this, preset: this.preset[i]}));
			// Preset item delete button
			let del=new St.Button({label: "[ × ]"});
	...
	on_slider_changed: function () {
		...
		this.periodMenuItem.setLabel("In:  " + (this.periodH < 10 ? "0" : "") + this.periodH + ":" + (this.periodM < 10 ? "0" : "") + this.periodM + ":" + (this.periodS < 10 ? "0" : "") + this.periodS);
		if (this.period) {
			...
			this.timeMenuItem.setLabel("At:  " + (h < 10 ? "0" : "") + h + ":" + (m < 10 ? "0" : "") + m + ":" + (s < 10 ? "0" : "") + s + (this.endTime.getTime() > today.getTime() + 86400000 ? " (tomorrow)" : ""));
	...
	_tick: function () {
		...
		if (this.period >= 0) {
			...
			this.periodMenuItem.setLabel("In:  " + label);
			let h = this.endTime.getHours();
			let m = this.endTime.getMinutes();
			let s = this.endTime.getSeconds();
			let today = new Date();
			...
			this.timeMenuItem.setLabel("At:  " + (h < 10 ? "0" : "") + h + ":" + (m < 10 ? "0" : "") + m + ":" + (s < 10 ? "0" : "") + s + (this.endTime.getTime() > today.getTime() + 86400000 ? " (tomorrow)" : ""));
			this._moveSlider();
			if (this.turnOffSwitchMenuItem.state && this.period < 10) this.set_applet_label("Turn off in " + this.periodS + "s");
			else this.set_applet_label(label);
			this.set_applet_tooltip("Timer: " + label);
			...
			if (this.turnOffSwitchMenuItem.state) {
				...
				this.flashCount = 0;
				this._flash();
				if (this.soundOn && this.soundPath) Util.spawnCommandLine("play " + this.soundPath);
				if (this.showNotifications) Util.spawnCommandLine('zenity --notification --text=\"Timer:  Time\'s up!\" --window-icon=clock');
			}
		}
	},
