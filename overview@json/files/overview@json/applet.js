...
class CinnamenuApplet extends TextIconApplet {
    constructor(metadata, orientation, panel_height, instance_id) {
        ...
        this.currentCategory = 'all'; // will be overridden from lastCategory on menu open
        ...
        this.signals.connect(this.appFavorites, 'changed', () => {
                        if (this.display) {// Check if display is initialised
                            ...
                            if (this.display.favoriteBox) this.display.favoriteBox.populate();
        ...
        const updateKeybinding = () => {
            Main.keybindingManager.addHotKey(
                'overview-key-' + this.instance_id,
                this.settings.overviewKey || '<Super>a',
                () => {
        ...
        { key: 'system-buttons-placement',  value: 'systemButtonsPlacement',cb: refreshDisplay },
        { key: 'sidebar-favorites',         value: 'sidebarFavorites',      cb: refreshDisplay },
        { key: 'startup-view',              value: 'startupView',           cb: null },
        { key: 'search-placement',          value: 'searchPlacement',       cb: refreshDisplay },
        { key: 'last-category',             value: 'lastCategory',          cb: null },
        ...
        { key: 'overview-key',              value: 'overviewKey',           cb: updateKeybinding },
        ...
        { key: 'show-fav-box',             value: 'showFavBox',            cb: refreshDisplay },
        { key: 'show-fav-box-avatar',      value: 'showFavBoxAvatar',      cb: refreshDisplay },
        { key: 'fav-box-avatar-size',      value: 'favBoxAvatarSize',      cb: refreshDisplay },
        { key: 'show-fav-box-apps',        value: 'showFavBoxApps',        cb: refreshDisplay },
        { key: 'fav-box-icon-size',        value: 'favBoxIconSize',        cb: refreshDisplay },
        { key: 'fav-box-sys-horizontal',   value: 'favBoxSysHorizontal',   cb: refreshDisplay },
        ...
        { key: 'overview-hot-corner',       value: 'overviewHotCorner',     cb: () => {
            if (this.overview) this.overview.setHotCorner(this.settings.overviewHotCorner);
        }}
        ...
        // Ensure tile-style defaults are set for users upgrading from older installs
        // where these keys may not yet exist in the prefs JSON.
        if (!this.settings.overviewTileStyle ||
                this.settings.overviewTileStyle === 'accent') {
            this.settings.overviewTileStyle = 'metro';
        }
        if (!this.settings.menuTileStyle ||
                this.settings.menuTileStyle === 'accent') {
            this.settings.menuTileStyle = 'metro';
        }
        if (this.settings.useMenuTileStyle === undefined) {
            this.settings.useMenuTileStyle = true;
        }
        ...
        this.overview.setHotCorner(this.settings.overviewHotCorner || 'none');
...
//----------------TextIconApplet callbacks----------------
    ...
    on_applet_removed_from_panel() {
        ...
        Main.keybindingManager.removeHotKey('overview-key-' + this.instance_id);
...
//==================================================================
    ...
    _onOpenStateToggled(menu, open) {
        ...
        if (open) {
            ...
            // Restore last-used category from persistent setting
            const savedCat = this.settings.lastCategory || 'all';
            this.currentCategory = savedCat;
            // Re-validate the restored category
            if (this.currentCategory === 'places' && !this.settings.showPlaces ||
                    this.currentCategory === 'recents' && !this.recentsEnabled ||
                    this.currentCategory === 'favorite_apps' && !this.settings.showFavAppsCategory ||
                    this.currentCategory.startsWith('emoji:') && !this.settings.showEmojiCategory) {
            ...
            // startupView: 0=last used, 1=favourite apps
            if (this.settings.startupView === 1) {
                openOnCategory = this.settings.showFavAppsCategory ? 'favorite_apps' : 'all';
    ...
    setActiveCategory(categoryId) {
        ...
        // Persist so startupView=0 (last used) survives Cinnamon restarts
        try { this.settings.lastCategory = categoryId; } catch(e) {}
