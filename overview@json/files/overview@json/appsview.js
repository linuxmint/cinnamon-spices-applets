...
class AppButton {
    ...
    _addTileStyle() {
         ...
         *
         * CSS classes emitted on the button actor:
         *   .tile-style                    — tile style is ON (any mode)
         *   .tile-light / .tile-dark       — built-in frosted palette (no inline colour)
         *   .tile-metro                    — Metro: inline background-color set by JS
         *   .tile-css                      — CSS-only: cinnamon.css owns all styling
         *
         * For light-palette / dark-palette the entire menu container also receives
         * .menu-palette-light or .menu-palette-dark (set in display.js) so the
         * stylesheet can style sidebar, search box, and background as a complete theme.
         */
        if (!this.appThis.settings.useMenuTileStyle) {
            return;
        }

        const mode = this.appThis.settings.menuTileStyle || 'light-palette';
        this.actor.add_style_class_name('tile-style');

        if (mode === 'light-palette') {
            this.actor.add_style_class_name('tile-light');
            this.actor.set_style(null);
            return;
        }

        if (mode === 'dark-palette') {
            this.actor.add_style_class_name('tile-dark');
            this.actor.set_style(null);
            return;
        }

        if (mode === 'css') {
            this.actor.add_style_class_name('tile-css');
            this.actor.set_style(null);
            return;
        }

        // Metro mode: macOS Monterey/Ventura system colour palette
        // Tiles are frosted glass: same rounded shape as tile-light/tile-dark
        // but with the macOS colour as a translucent tint.
        this.actor.add_style_class_name('tile-metro');
        const MACOS_COLORS = [
        ...
        const getColor = (app) => {
            let h = 5381;
            const s = (app.id || app.name || '');
            for (let i = 0; i < s.length; i++) { h = ((h << 5) + h) ^ s.charCodeAt(i); h |= 0; }
            return MACOS_COLORS[Math.abs(h) % MACOS_COLORS.length];
        };
        // Parse hex to rgb
        const hexToRgb = (hex) => ({
        ...
        const color = getColor(this.app);
        const c = hexToRgb(color);
        // Frosted glass: translucent colour bg + white border, same border-radius as palette tiles
        if (this.has_focus) {
            this.actor.set_style(
                `background-color:rgba(${c.r},${c.g},${c.b},0.85);` +
                `border-radius:10px;border:2px solid rgba(255,255,255,0.55);padding:5px 1px;`
            );
        } else {
            this.actor.set_style(
                `background-color:rgba(${c.r},${c.g},${c.b},0.72);` +
                `border-radius:10px;border:1px solid rgba(255,255,255,0.35);padding:6px 2px;`
            );
        }
    }
