...
class WorkspaceButton {
    ...
    show() {
        this.actor.connect('button-release-event', this.onClicked.bind(this));
...
class SimpleButton extends WorkspaceButton {
    constructor(index, applet) {
        ...
        this.style = {
            background_active: applet.background_active,
            background_inactive: applet.background_inactive,
            inactive_opacity: applet.inactive_opacity,
            border_size: applet.border_size,
            border_color: applet.border_color,
            text_color_active: applet.text_color_active,
            text_color_inactive: applet.text_color_inactive
        };

        // Add alpha parameter 
        this.style.background_inactive = this.style.background_inactive.substring(0, this.style.background_inactive.indexOf(')'))+", "+this.style.inactive_opacity+")";
        this.style.background_inactive = this.style.background_inactive.replace("rgb", "rgba");
        // global.logError("background_inactive: "+this.style.background_inactive);
       
        // override the background color of the button from the default one to the background_inactive defined
        // by the user 
        this.actor.set_style(`
            background-color: ${this.style.background_inactive};
            border: ${this.style.border_size}px solid ${this.style.border_color};
            color:  ${this.style.text_color_inactive};
        `);
    ...
    activate(active, style2=null) {
        if (active) {
            this.actor.set_style(`
                background-color: ${this.style.background_active};
                border: ${this.style.border_size}px solid ${this.style.border_color};
                color:  ${this.style.text_color_active};
            `);
        }else{
            this.actor.set_style(`
                background-color: ${this.style.background_inactive};
...
class CinnamonWorkspaceSwitcher extends Applet.Applet {
    constructor(metadata, orientation, panel_height, instance_id) {
        ...
        // global.logError("[WS] Applet UUID: " + metadata.uuid);
        // global.logError("[WS] Applet path: " + metadata.path);
        ...
        this.settings.bind("background-active", "background_active", this.queueCreateButtons);
        this.settings.bind("background-inactive", "background_inactive", this.queueCreateButtons);
        this.settings.bind("border-size", "border_size", this.queueCreateButtons);
        this.settings.bind("border-color", "border_color", this.queueCreateButtons);
        this.settings.bind("inactive-opacity", "inactive_opacity", this.queueCreateButtons);
        this.settings.bind("text-color-active", "text_color_active", this.queueCreateButtons);
        this.settings.bind("text-color-inactive", "text_color_inactive", this.queueCreateButtons);
        ...
        global.settings.connect('changed::panel-edit-mode', this.on_panel_edit_mode_changed.bind(this));

        let expoMenuItem = new PopupMenu.PopupIconMenuItem(_("Manage workspaces (Expo)"), "xsi-view-grid-symbolic", St.IconType.SYMBOLIC);
        expoMenuItem.connect('activate', () => {
        ...
        addWorkspaceMenuItem.connect('activate', () => {
            Main._addWorkspace();
        });
    ...
    onResetAppearance(){
        // global.log("[WSP] Reset to Default Style");
        this.settings.setValue("border-color",this.settings.settingsData["border-color"].default);
        this.settings.setValue("border-size",this.settings.settingsData["border-size"].default);
        this.settings.setValue("background-active",this.settings.settingsData["background-active"].default);
        this.settings.setValue("background-inactive",this.settings.settingsData["background-inactive"].default);
        this.settings.setValue("inactive-opacity",this.settings.settingsData["inactive-opacity"].default);
        this.settings.setValue("text-color-inactive",this.settings.settingsData["text-color-inactive"].default);
        this.settings.setValue("text-color-active",this.settings.settingsData["text-color-active"].default);
        this.queueCreateButtons();
    }
    ...
    _onWorkspaceChanged(wm, from, to) {
        this.buttons[from].activate(false, this.style);
        this.buttons[to].activate(true, this.style);
    }
    ...
    queueCreateButtons() {
        if (!this.createButtonsQueued) {
            Mainloop.idle_add(this._createButtons.bind(this));
