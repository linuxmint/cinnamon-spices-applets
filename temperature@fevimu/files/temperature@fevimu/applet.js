...
MyApplet.prototype = {
  ...
  _update_temp: function() {
    ...
    if (!tempInfo) {
      ...
      if (tempInfo.temp) {
        ...
        if (tempInfo.crit) items.push(_('Critical Temperature : ') + this._formatTemp(tempInfo.crit));
      }
    }
  ...
  _formatTemp: function(t) {
    ...
    else return (Math.round(t * 10) / 10).toFixed(precisionDigits).toString() + ' °C';
  }
};
