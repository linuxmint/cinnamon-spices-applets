...
const UUID = "cwgMintMenu@clockworkgobin.org";
...
SystemButton.prototype = {
    ...
    _init: function(appsMenuButton, icon, nbFavorites, name, desc) {
        ...
        //this.actor.style = "padding-top: "+(icon_size / 3)+"px;padding-bottom: "+(icon_size / 3)+"px; margin:auto;"
        this.actor.add_style_class_name('menu-favorites-button');
	this.actor.style = "padding: 4px; margin-left: 4px;";
...
SearchBox.prototype = {
    _init: function() {
	this.actor = new St.BoxLayout({ style_class: 'menu-search-box' });
        this.actor._delegate = this;
    }
}
...
MyApplet.prototype = {
    ...
    _init: function(orientation, panel_height, instance_id) {        
        ...
        try {
            ...
            this.settings = new Settings.AppletSettings(this, "cwgMintMenu@clockworkgobin.org", instance_id);

            this.settings.bindProperty(Settings.BindingDirection.IN, "show-places", "showPlaces", this._refreshBelowApps, null);

            this.settings.bindProperty(Settings.BindingDirection.IN, "system-bar-show", "systemBarShow", this._toggleSystemBar, null);
    ...
    _refreshFavs : function() {
        ...
        for ( let i = 0; i < launchers.length; ++i ) {
            ...
            if (app) {
                let button = new FavoritesButton(this, app, launchers.length); // launchers.length + 3 because we're adding 3 system buttons at the bottom
	...
	let rsearch = new SearchBox();
	this.SearchBox = rsearch.actor;
	...
	if (this.systemBarShow) {
	        this.systemButtonBar = new St.BoxLayout({ vertical: false });
		//System Settings (Control Center)
		button = new SystemButton(this, "applications-system", 1, _("System Settings"), _("Control Center"));
		this._addEnterEvent(button, Lang.bind(this, this._favEnterEvent, button));
		button.actor.connect('leave-event', Lang.bind(this, this._favLeaveEvent, button));
		...
		this.systemButtonBar.add_actor(button.actor, { y_align: St.Align.END, y_fill: false });

		//Lock screen
		button = new SystemButton(this, "system-lock-screen", 1, _("Lock screen"), _("Lock the screen"));
		this._addEnterEvent(button, Lang.bind(this, this._favEnterEvent, button));
		button.actor.connect('leave-event', Lang.bind(this, this._favLeaveEvent, button));
		button.activate = Lang.bind(this, function() {
		    this.menu.close();
		...
		//Logout button
		button = new SystemButton(this, "system-log-out", 1, _("Logout"), _("Leave the session"));
		this._addEnterEvent(button, Lang.bind(this, this._favEnterEvent, button));
		button.actor.connect('leave-event', Lang.bind(this, this._favLeaveEvent, button));
		button.activate = Lang.bind(this, function() {
		    this.menu.close();
		...
		//Suspend button
		button = new SystemButton(this, "gnome-session-suspend", 1, _("Suspend"), _("Put the computer to sleep"));
		this._addEnterEvent(button, Lang.bind(this, this._favEnterEvent, button));
		button.actor.connect('leave-event', Lang.bind(this, this._favLeaveEvent, button));
		button.activate = Lang.bind(this, function() {
		    this.menu.close();
		...
		//Shutdown button
		button = new SystemButton(this, "system-shutdown", 1, _("Quit"), _("Shutdown the computer"));
		this._addEnterEvent(button, Lang.bind(this, this._favEnterEvent, button));
		button.actor.connect('leave-event', Lang.bind(this, this._favLeaveEvent, button));
		button.activate = Lang.bind(this, function() {
		    this.menu.close();
		...
		this.systemButtonBar.add_actor(button.actor, { y_align: St.Align.END, y_fill: false });
		this.SearchBox.add_actor(this.systemButtonBar, { y_align: St.Align.END, y_fill: false });
	}
	...
	this.sbIter = new VisibleChildIterator(this, this.systemButtonBar);
	this.systemButtonBar._vis_iter = this.sbIter;
    ...
    _toggleSystemBar : function() {
	if (this.systemBarShow) {
		this.systemButtonBar.show();
	}
	else {
		this.systemButtonBar.hide();
	}
    }
};
