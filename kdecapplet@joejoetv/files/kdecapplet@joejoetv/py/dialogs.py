...
class SendURLDialog(Gtk.Dialog):
    def __init__(self, device_name):
        super().__init__(
            title=_("Send URL to '{device_name}'").format(device_name=device_name),
            flags=0,
        )
        ...
        ok_button = Gtk.Button.new_from_icon_name(
        ...
        url_label = Gtk.Label(
            label=_("Enter a URL to send to '{device_name}'").format(
...
class SendSMSDialog(Gtk.Dialog):
    def __init__(self, device_name):
        super().__init__(
            title=_("Send SMS using '{device_name}'").format(device_name=device_name),
            flags=0,
        )
...
class SendTextDialog(Gtk.Dialog):
    def __init__(self, device_name):
        super().__init__(
            title=_("Send text to '{device_name}'").format(device_name=device_name),
            flags=0,
        )
...
class OpenDirectoryDialog(Gtk.Dialog):
    ...
    def __init__(
        self, device_name: str, mount_point: str, directories: list[tuple[str, str]]
    ):
        super().__init__(
            title=_("Open remote directory on '{device_name}'").format(
                device_name=device_name
            ),
            flags=0,
        )
    ...
    def _create_list_widget(self, item: DirListItem):
        ...
        icon = Gtk.Image.new_from_icon_name(
            "folder-symbolic", Gtk.IconSize.LARGE_TOOLBAR
        )
        ...
        path_label.set_markup(
            f"<span size='smaller'>{item.path.removeprefix(self.mount_point)}</span>"
        )
    ...
    if len(sys.argv) >= 3:
        if sys.argv[1] == "sendfiles":
            ...
            filechooserdialog = Gtk.FileChooserNative(
                title=_("Select files to send to '{deviceName}'").format(
            ...
            filechooserdialog = Gtk.FileChooserNative(
                title=_(
                    "Select where to save photo received from '{deviceName}'"
