...
const { St, GLib, Gio, GObject, GdkPixbuf, Gdk, Pango } = imports.gi;
let Cairo = null;
...
class CreaturePopupMenu extends Applet.AppletPopupMenu {
    ...
    _rebuildVisualList() {
        ...
        this._addCopyBank(MATH_SYMBOL_BANK);
    }
...
class CreatureApplet extends Applet.Applet {
    ...
    feedCreatureWithItem(item) {
        ...
        let reaction = (item.emoji || "🍽") + " " + (item.reaction || "Merci pour le repas !");
    ...
    cleanCreatureWithItem(item) {
        ...
        let reaction = (item.emoji || "🧽") + " " + (item.reaction || "Je ne sais pas quoi en penser.");
        this.state.lastMessage = reaction;
    ...
    _feedChoices(profile) {
        ...
        let bad = shuffledCopy(BAD_FOOD_ITEMS).slice(0, 5 - good.length);
   ...
   _buildQuotePool(profile) {
    ...
    _refreshUi() {
        ...
        let hearts = friendshipHearts(this.state.friendshipLevel);
    ...
    _moodText() {
        ...
        if (hour >= 23 || hour < 6) {
            return "😴 Tu travailles tard… je suis fatigué.";
        }
        if ((this.state.cleanliness || 100) < 35) {
            return "🫧 Je commence à sentir le vieux tiroir.";
        }
        if ((this.state.hunger || 100) < 35) {
            return "🥺 J’ai faim… vraiment faim.";
        }
        if (this._dailyCareMoodEnabled() && !fedToday) {
            return "🥺 Je n’ai pas encore mangé aujourd’hui.";
        }
        if (idle > 3600) {
            return "🥺 Ça fait longtemps… tu m’avais oublié ?";
        }
        if (fedToday) {
            return "🥰 Merci pour les snacks.";
        }
        return "🙂 Je surveille le panneau tranquillement.";
    }
        ...
        try {
            ...
            if (ok) {
                ...
                if (!isNaN(percent) && percent <= 20) {
                    event = "🔋 Batterie faible… je vais économiser mes plumes.";
                }
            }
        } catch (e) {}
        ...
        if (!event) {
            try {
                ...
                if (ok2 && ByteArray.toString(out2).trim() !== "connected") {
                    event = "📡 Wi‑Fi perdu… je capte seulement ton affection.";
                }
            } catch (e2) {}
        }
