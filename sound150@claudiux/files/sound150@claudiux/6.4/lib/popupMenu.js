...
var PopupBaseMenuItem = class PopupBaseMenuItem {
    ...
    _init(params) {
        ...
        this._signals.connect(this.actor, 'get-preferred-width', (actor, forHeight, alloc) => { this._getPreferredWidth(actor, forHeight, alloc) });
        this._signals.connect(this.actor, 'get-preferred-height', (actor, forWidth, alloc) => { this._getPreferredHeight(actor, forWidth, alloc) });
        this._signals.connect(this.actor, 'allocate', (actor, box, flags) => { this._allocate(actor, box, flags) });
        this._signals.connect(this.actor, 'style-changed', (actor) => { this._onStyleChanged(actor) });
        ...
        if (this._activatable) {
            this._signals.connect(this.actor, 'button-release-event', (actor, event) => { this._onButtonReleaseEvent(actor, event) });
            this._signals.connect(this.actor, 'key-press-event', (actor, event) => { this._onKeyPressEvent(actor, event) });
        }
        if (params.reactive && params.hover)
            this._signals.connect(this.actor, 'notify::hover', (actor) => { this._onHoverChanged(actor) });
        if (params.reactive) {
            this._signals.connect(this.actor, 'key-focus-in', (actor) => { this._onKeyFocusIn(actor) });
            this._signals.connect(this.actor, 'key-focus-out', (actor) => { this._onKeyFocusOut(actor) });
        }
    }
    ...
    setShowDot(show) {
        if (show) {
            ...
            this._signals.connect(this._dot, 'repaint', (area) => { this._onRepaintDot(area) });
...
var PopupSeparatorMenuItem = class PopupSeparatorMenuItem extends PopupBaseMenuItem {
    _init () {
        ...
        this._signals.connect(this._drawingArea, 'repaint', (area) => { this._onRepaint(area) });
    }
...
var PopupSliderMenuItem = class PopupSliderMenuItem extends PopupBaseMenuItem {
    _init(value) {
        ...
        this._signals.connect(this._slider, 'repaint', (area) => { this._sliderRepaint(area) });
        this._signals.connect(this.actor, 'button-press-event', (actor, event) => { this._startDragging(actor, event) });
        this._signals.connect(this.actor, 'scroll-event', (actor, event) => { this._onScrollEvent(actor, event) });
    ...
    _startDragging(actor, event) {
        ...
        this._signals.connect(this._slider, 'motion-event', () => { this._motionEvent() });
        let absX, absY;
        ...
        if (this.shellItem != shellItem) {
            ...
            if (this.shellItem) {
                ...
                this._shellItemSignalsHandlers = this._connectAndSaveId(this.shellItem, {
                    ...
                    'destroy' :  () => { this._onShellItemDestroyed() }
                });
                ...
                if (this.shellItem.menu) {
                    ...
                    this._shellMenuSignalsHandlers = this._connectAndSaveId(this.shellItem.menu, {
                        'open-state-changed': () => { this._onOpenStateChanged() },
                        'destroy'           : () => { this._onShellMenuDestroyed() }
                    });
                } else {
...
var PopupMenu = class PopupMenu extends PopupMenuBase {
    ...
    _init(sourceActor, orientation) {
        ...
        this._signals.connect(this._boxWrapper, 'get-preferred-width', () => { this._boxGetPreferredWidth() });
        this._signals.connect(this._boxWrapper, 'get-preferred-height', () => { this._boxGetPreferredHeight() });
        this._signals.connect(this._boxWrapper, 'allocate', () => { this._boxAllocate() });
        this._signals.connect(this.actor, 'notify::allocation', () => { this._allocationChanged() });
...
var PopupSubMenuMenuItem = class PopupSubMenuMenuItem extends PopupBaseMenuItem {
    _init(text) {
        ...
        this._signals.connect(this.menu, 'open-state-changed', (menu, open) => { this._subMenuOpenStateChanged(menu, open) });
    }
...
var PopupComboMenu = class PopupComboMenu extends PopupMenuBase {
    _init(sourceActor) {
        ...
        this.actor._delegate = this;
        this._signals.connect(this.actor, 'key-press-event', (actor, event) => { this._onKeyPressEvent(actor, event) });
        this._signals.connect(this.actor, 'key-focus-in', (actor) => { this, this._onKeyFocusIn(actor) });
...
var PopupMenuFactory = class PopupMenuFactory {
    ...
    _attachToMenu(shellItem, factoryItem) {
        ...
        factoryItem.setShellItem(shellItem, {
            'child-added'   : () => { this._onChildAdded() },
            'child-moved'   : () => { this._onChildMoved() }
        });
        this._menuLikend.push(factoryItem);
        factoryItem.connectAndRemoveOnDestroy({
            'destroy'           : () => { this._onDestroyMainMenu() }
        });
    }
    ...
    _createItem(factoryItem) {
        ...
        GLib.idle_add(() => this._createChildrens(factoryItem));

        // Now, connect various events
        factoryItem.setShellItem(shellItem, {
            'type-changed':       () => { this._onTypeChanged() },
            'child-added':        () => { this._onChildAdded() },
            'child-moved':        () => { this._onChildMoved() }
        });
        return shellItem;
