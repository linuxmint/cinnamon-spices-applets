...
MyApplet.prototype = {
    ...
    _init: function(orientation, panel_height) {        
        ...
        try {        
            ...
            this.set_applet_tooltip(_("Display & Layouts"));
    ...
    _createMenu: function() {
        this._createConfigurationsMenu();
        ...
        this.menu.addAction(_("Configure display layouts..."), function() {
            GLib.spawn_command_line_async('arandr');
        });
    },

    _createConfigurationsMenu: function() {
        let confDir =  Gio.file_new_for_path(GLib.get_home_dir()+"/.screenlayout");
        if (confDir.query_exists(null)){
            let header = new PopupMenu.PopupMenuItem(_("Display Layouts"));
            header.label.add_style_class_name('display-subtitle');
            this.menu.addMenuItem(header);

            let enumerator= confDir.enumerate_children(Gio.FILE_ATTRIBUTE_STANDARD_NAME,
                                                       Gio.FileQueryInfoFlags.NONE,null);
            let file;
            while ( (file = enumerator.next_file(null)) != null) {
                let fileName = file.get_name();
                if ( fileName.split(".").pop() == "sh") {
                    let label = fileName.replace(/\.[^/.]+$/, "");
                    label = label.charAt(0).toUpperCase() + label.slice(1);
                    this.menu.addAction(label, function() {
                        let cmd = "sh " + confDir.get_path() + "/" + fileName;
    ...
    _addOutputItem: function(config, output) {
        let popup_menu_item = new PopupMenu.PopupSubMenuMenuItem(output.get_display_name());
        popup_menu_item.label.add_style_class_name('display-subtitle');
        ...
        for (let i = 0; i < rotations.length; i++) {
            ...
            if (bitmask & allowedRotations) {
                ...
                popup_menu_item.menu.addMenuItem(item);
            }
        }
        this.menu.addMenuItem(popup_menu_item);
    },
