...
// Uses GLib.DateTime.format() — same strftime strings, no process spawning,
// completely non-blocking. This is the same approach Cinnamon's built-in clock uses.

function formatNow(fmt) {
    try {
        return GLib.DateTime.new_now_local().format(fmt) || "";
    } catch(e) {
        global.logError("[compact-clock@kb] formatNow: " + e);
        return "";
    }
}

// ─── Parse font family from a Pango font description string ───────────────────
// fontchooser stores e.g. "JetBrains Mono Bold 12" — we only want the family.

function parseFontFamily(desc) {
    if (!desc) return "JetBrains Mono";
    // Strip trailing point-size (integer or decimal)
    let s = desc.replace(/\s+\d+(\.\d+)?\s*$/, "");
    // Strip trailing common style / weight keywords
    s = s.replace(
        /\s+(Bold|Italic|Regular|Light|Medium|Heavy|Black|Thin|ExtraLight|ExtraBold|SemiBold|Condensed|Expanded|Oblique|Mono)\s*$/gi,
        ""
    );
    return s.trim() || "JetBrains Mono";
}

// ─── Make a label that never ellipsizes ──────────────────────────────────────
// This lets the label drive its own natural width so the applet
// resizes to fit its content instead of clipping text.
...
CompactClockApplet.prototype = {
    ...
    _init(metadata, orientation, panelHeight, instanceId) {
        ...
        // Left: day-of-week (large, fills panel height)
        ...
        // Right: time (top) + date (bottom), right-aligned
    ...
    _tick() {
        ...
        // Inject seconds into the time format if requested
        if (this._showSeconds && !timeFmt.includes("%S")) {
            timeFmt = timeFmt.replace("%M", "%M:%S");
        }

        let timeStr = formatNow(timeFmt);
        let dateStr = formatNow(dateFmt);
        let dayStr  = formatNow(dayFmt).toUpperCase();
    ...
    _scheduleNext() {
        ...
        // Every second when showing seconds; every 5 s otherwise
