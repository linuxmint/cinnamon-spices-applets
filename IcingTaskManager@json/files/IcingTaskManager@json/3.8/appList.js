...
const {each, findIndex, find, isEqual, setTimeout, unref} = require('./utils');
...
class AppList {
  constructor(params) {
    ...
    this.signals.connect(this.metaWorkspace, 'window-added', (...args) => this._windowAdded(...args));
    this.signals.connect(this.metaWorkspace, 'window-removed', (...args) => this._windowRemoved(...args));
  ...
  _windowAdded (metaWorkspace, metaWindow, app, isFavoriteApp) {
    if (!this.state) {
      return;
    }
