...
const AppletMeta = imports.ui.appletManager.applets["GnomeApplications@bownz"];
const AppletDir = imports.ui.appletManager.appletMeta["GnomeApplications@bownz"].path;
...
const CATEGORY_ICON_SIZE = menuSettings.get_int("icon-size");
const APPLICATION_ICON_SIZE = menuSettings.get_int("sub-icon-size");
...
ApplicationButton.prototype = {
    ...
    _init: function(appsMenuButton, app) {
        ...
        this.actor.add_style_class_name('menu-application-button-label');
...
MyApplet.prototype = {
    ...
    _init: function(orientation) {        
	    ...
	    this.set_applet_label(_("Applications"));
            ...
            this.menu.connect('open-state-changed', Lang.bind(this, this._onSearchTextChanged));                                
            ...
            this.edit_menu_item = new Applet.MenuItem(_("Edit Applications"), Gtk.STOCK_EDIT, Lang.bind(this, this._launch_editor));
    ...
    _onOpenStateToggled: function(menu, open) {
       if (open) {
           ...
           let applicationsBoxHeight = this.applicationsBox.get_allocation_box().y2-this.applicationsBox.get_allocation_box().y1;
            this.applicationsScrollBox.style = "height: 50px;";
       } else {
    ...
    _display : function() {
        ...
        this.leftBox = new St.BoxLayout({ style_class: 'menu-applications-scrollbox', vertical: true });        
        ...
        this.categoriesBox = new St.BoxLayout({ style_class: 'menu-applications-scrollbox', vertical: true });
        this.applicationsScrollBox = new St.ScrollView({ x_fill: true, y_fill: false, y_align: St.Align.START, style_class: 'menu-applications-scrollbox' });
    ...
    _clearApplicationsBox: function(selectedActor){
       ...
       let actors = this.categoriesBox.get_children();
         ...
         for (var i=0; i<actors.length; i++){
             let actor = actors[i];
            if (actor==selectedActor) actor.style_class = "menu-category-button-selected";
    ...
    _onApplicationButtonRealized: function(actor) {
        if (actor.get_width() > this._applicationsBoxWidth){
            this._applicationsBoxWidth = 250
