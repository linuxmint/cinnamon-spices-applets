/*
* Window List Hotkey
* Main Cinnamon window list with hotkey (SUPER + NUMBER).
* Copyright (C) 2015 Putu Henry Nugraha
* Putu.Henry.Nugraha@gmail.com
*
...
MyApplet.prototype = {
    ...
    _init: function(orientation, panel_height) {        
        ...
        try {                    
			...
			// Preparing an array to store list of windows individually for each workspaces.
			this._workspace_windows = new Array(256); // Assuming there will only be 256 workspaces.
			for ( let i = 0; i < 256; i++ ){
			  this._workspace_windows[i] = new Array()
			}
            // Bind hotkey for first 10 windows.
			Main.keybindingManager.addHotKey("hotkey-window-1", "<Super>1", Lang.bind(this, function() {
				let ws_index = global.screen.get_active_workspace().index();
				if ( this._workspace_windows[ws_index][0].minimized ) {
					this._workspace_windows[ws_index][0].unminimize( global.get_current_time() );
					Main.activateWindow( this._workspace_windows[ws_index][0], global.get_current_time() );
				} else if ( this._workspace_windows[ws_index][0].has_focus() == false ) {
					Main.activateWindow( this._workspace_windows[ws_index][0], global.get_current_time() );
				} else {
					this._workspace_windows[ws_index][0].minimize( global.get_current_time() );
				}
				return;
			}));
			Main.keybindingManager.addHotKey("hotkey-window-2", "<Super>2", Lang.bind(this, function() {
				let ws_index = global.screen.get_active_workspace().index();
				if ( this._workspace_windows[ws_index][1].minimized ) {
					this._workspace_windows[ws_index][1].unminimize( global.get_current_time() );
					Main.activateWindow( this._workspace_windows[ws_index][1], global.get_current_time() );
				} else if ( this._workspace_windows[ws_index][1].has_focus() == false ) {
					Main.activateWindow( this._workspace_windows[ws_index][1], global.get_current_time() );
				} else {
					this._workspace_windows[ws_index][1].minimize( global.get_current_time() );
				}
				return;
			}));
			Main.keybindingManager.addHotKey("hotkey-window-3", "<Super>3", Lang.bind(this, function() {
				let ws_index = global.screen.get_active_workspace().index();
				if ( this._workspace_windows[ws_index][2].minimized ) {
					this._workspace_windows[ws_index][2].unminimize( global.get_current_time() );
					Main.activateWindow( this._workspace_windows[ws_index][2], global.get_current_time() );
				} else if ( this._workspace_windows[ws_index][2].has_focus() == false ) {
					Main.activateWindow( this._workspace_windows[ws_index][2], global.get_current_time() );
				} else {
					this._workspace_windows[ws_index][2].minimize( global.get_current_time() );
				}
				return;
			}));
			Main.keybindingManager.addHotKey("hotkey-window-4", "<Super>4", Lang.bind(this, function() {
				let ws_index = global.screen.get_active_workspace().index();
				if ( this._workspace_windows[ws_index][3].minimized ) {
					this._workspace_windows[ws_index][3].unminimize( global.get_current_time() );
					Main.activateWindow( this._workspace_windows[ws_index][3], global.get_current_time() );
				} else if ( this._workspace_windows[ws_index][3].has_focus() == false ) {
					Main.activateWindow( this._workspace_windows[ws_index][3], global.get_current_time() );
				} else {
					this._workspace_windows[ws_index][3].minimize( global.get_current_time() );
				}
				return;
			}));
			Main.keybindingManager.addHotKey("hotkey-window-5", "<Super>5", Lang.bind(this, function() {
				let ws_index = global.screen.get_active_workspace().index();
				if ( this._workspace_windows[ws_index][4].minimized ) {
					this._workspace_windows[ws_index][4].unminimize( global.get_current_time() );
					Main.activateWindow( this._workspace_windows[ws_index][4], global.get_current_time() );
				} else if ( this._workspace_windows[ws_index][4].has_focus() == false ) {
					Main.activateWindow( this._workspace_windows[ws_index][4], global.get_current_time() );
				} else {
					this._workspace_windows[ws_index][4].minimize( global.get_current_time() );
				}
				return;
			}));
			Main.keybindingManager.addHotKey("hotkey-window-6", "<Super>6", Lang.bind(this, function() {
				let ws_index = global.screen.get_active_workspace().index();
				if ( this._workspace_windows[ws_index][5].minimized ) {
					this._workspace_windows[ws_index][5].unminimize( global.get_current_time() );
					Main.activateWindow( this._workspace_windows[ws_index][5], global.get_current_time() );
				} else if ( this._workspace_windows[ws_index][5].has_focus() == false ) {
					Main.activateWindow( this._workspace_windows[ws_index][5], global.get_current_time() );
				} else {
					this._workspace_windows[ws_index][5].minimize( global.get_current_time() );
				}
				return;
			}));
			Main.keybindingManager.addHotKey("hotkey-window-7", "<Super>7", Lang.bind(this, function() {
				let ws_index = global.screen.get_active_workspace().index();
				if ( this._workspace_windows[ws_index][6].minimized ) {
					this._workspace_windows[ws_index][6].unminimize( global.get_current_time() );
					Main.activateWindow( this._workspace_windows[ws_index][6], global.get_current_time() );
				} else if ( this._workspace_windows[ws_index][6].has_focus() == false ) {
					Main.activateWindow( this._workspace_windows[ws_index][6], global.get_current_time() );
				} else {
					this._workspace_windows[ws_index][6].minimize( global.get_current_time() );
				}
				return;
			}));
			Main.keybindingManager.addHotKey("hotkey-window-8", "<Super>8", Lang.bind(this, function() {
				let ws_index = global.screen.get_active_workspace().index();
				if ( this._workspace_windows[ws_index][7].minimized ) {
					this._workspace_windows[ws_index][7].unminimize( global.get_current_time() );
					Main.activateWindow( this._workspace_windows[ws_index][7], global.get_current_time() );
				} else if ( this._workspace_windows[ws_index][7].has_focus() == false ) {
					Main.activateWindow( this._workspace_windows[ws_index][7], global.get_current_time() );
				} else {
					this._workspace_windows[ws_index][7].minimize( global.get_current_time() );
				}
				return;
			}));
			Main.keybindingManager.addHotKey("hotkey-window-9", "<Super>9", Lang.bind(this, function() {
				let ws_index = global.screen.get_active_workspace().index();
				if ( this._workspace_windows[ws_index][8].minimized ) {
					this._workspace_windows[ws_index][8].unminimize( global.get_current_time() );
					Main.activateWindow( this._workspace_windows[ws_index][8], global.get_current_time() );
				} else if ( this._workspace_windows[ws_index][8].has_focus() == false ) {
					Main.activateWindow( this._workspace_windows[ws_index][8], global.get_current_time() );
				} else {
					this._workspace_windows[ws_index][8].minimize( global.get_current_time() );
				}
				return;
			}));
			Main.keybindingManager.addHotKey("hotkey-window-10", "<Super>10", Lang.bind(this, function() {
				let ws_index = global.screen.get_active_workspace().index();
				if ( this._workspace_windows[ws_index][9].minimized ) {
					this._workspace_windows[ws_index][9].unminimize( global.get_current_time() );
					Main.activateWindow( this._workspace_windows[ws_index][9], global.get_current_time() );
				} else if ( this._workspace_windows[ws_index][9].has_focus() == false ) {
					Main.activateWindow( this._workspace_windows[ws_index][9], global.get_current_time() );
				} else {
					this._workspace_windows[ws_index][9].minimize( global.get_current_time() );
				}
				return;
			}));
        }
    ...
    _windowAdded: function(metaWorkspace, metaWindow) {
        ...
        // Add to list of windows in current workspace
        this._workspace_windows[metaWorkspace.index()].push(metaWindow)
    },

    _windowRemoved: function(metaWorkspace, metaWindow) {
        // Remove window from current list
        let ws_index = metaWorkspace.index()
        for ( let i=0; i<this._workspace_windows[ws_index].length; ++i ) {
            if ( this._workspace_windows[ws_index][i] == metaWindow ) {
                this._workspace_windows[ws_index].splice(i, 1);
                break;
            }
        }
        for ( let i=0; i<this._windows.length; ++i ) {
