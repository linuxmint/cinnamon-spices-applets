...
class PlaceInfo {
    constructor(kind, file, name, icon) {
        ...
        // needed try-catch.
        this.icon = icon ? new Gio.ThemedIcon({ name: icon }) : icon !== null ? this.getIcon() : null;
    }
...
class PlacesManager {
    constructor() {
        ...
        this.places.special.push(
            new PlaceInfo(  'special', Gio.File.new_for_path(homePath), _('Home') ) );
        ...
        for (let i = 0, len = DEFAULT_DIRECTORIES.length; i < len; i++) {
            ...
            if (specialPath) {
                ...
                this.places.special.push( new PlaceInfo( 'special', Gio.File.new_for_path(specialPath) ) );
            }
        }
