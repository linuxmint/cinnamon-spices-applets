...
const CinnamenuPanel = require('./panel');
...
CinnamenuButton.prototype = {
  ...
  _init: function(metadata, orientation, panel_height, instance_id) {
    ...
    this.apps = [];
    let apps = ['chromium-browser', 'google-chrome', 'firefox', 'opera', 'midori'];
    for (let i = 0, len = apps.length; i < len; i++) {
      let app = this.appSystem.lookup_desktop_wmclass(apps[i]);
      let icon = null;
      if (app) {
...
let [metadata, orientation, panel_height, instance_id] =  global['Cinnamenu@json'];
