...
class SettingsWindow(Gtk.Window):
    ...
    def _get_timezones():
        ...
        now = dt.now()
        year = now.year + 1
        summer_day = dt(year, 7, 4, 0, 1, 1)
        winter_day = dt(year, 12, 25, 0, 1, 1)
        for p in all_timezones:
            zone = tz.timezone(p)
            #inf  = zone.localize(now, is_dst=None)
            #timezones.append(inf.tzname())
            #timezones.append(p)
            summer = zone.localize(summer_day, is_dst=None)
            winter = zone.localize(winter_day, is_dst=None)
            
            zone_text = p
            if summer.tzname() != winter.tzname():
                zone_text += ', ' + winter.tzname() + '/' + summer.tzname()
            elif summer.tzname() != p:
                zone_text += ', ' + summer.tzname()
            
            timezones.append(zone_text)

        #timezones = list(set(sorted(timezones)))
        for p in timezones:
            print p
    ...
    def _on_tz_edited(self, widget, path, new_value):
        arr = new_value.split(',');
        self.liststore_worldclocks[path][1] = arr[0]
        return
...
class CellRendererAutoComplete(Gtk.CellRendererText):
    ...
    def __init__(
        ...
        #self.completion.connect('cursor-on-match', self.on_match_selected)
    ...
    def on_match_selected(self, completion, model, iter):
        text = model[iter][0]
        arr  = text.split(',')
        print 'Match selected: ', arr[0]
        entry = completion.get_entry();
        entry.set_text(arr[0]);
        return True
    ...
    def do_start_editing(
        ...
        self.entry.set_text(saved_text)
        #entry.connect('editing-done', self.editing_done, path)
        self.entry.connect('focus-out-event', self.focus_out, path)
    ...
    def focus_out(self, entry, event, path):
        ...
        if self.force_match and new_value not in self._zonenames:
            print 'value not in list: ', new_value
            return
