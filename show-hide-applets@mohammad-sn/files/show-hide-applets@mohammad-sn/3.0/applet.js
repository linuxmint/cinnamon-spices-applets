...
MyApplet.prototype = {
  ...
  _init: function (metadata, orientation, panel_height, instance_id) {
    ...
    try {
      ...
      this.settings.bindProperty(
        ...
        Lang.bind(this, function () {
          if (this._hideTimeoutId && !this.auto_hide) {
            ...
            this._hideTimeoutId = null;
          } else if (this.auto_hide && this.h) this.autodo(true);
      ...
      this.settings.bindProperty(
        ...
        Lang.bind(this, function () {
          if (!this.statusintooltip) {
            ...
            if (this.auto_hide) this.set_applet_tooltip(_("Autohide ON"));
      ...
      this.panelEditMode = new PopupMenu.PopupSwitchMenuItem(
        _("Panel Edit mode"),
        editMode,
      );
      ...
      let addapplets = new PopupMenu.PopupMenuItem(
      ...
      addapplets.connect(
        ...
        Lang.bind(this, function () {
          Util.spawnCommandLine(
            "cinnamon-settings applets panel" + this.panel.panelId,
          );
        }),
      );
      ...
      if (!this.disable_starttime_autohide || this.auto_hide) {
        this._hideTimeoutId = Mainloop.timeout_add_seconds(
          2,
          Lang.bind(this, function () {
            if (this.h) this.autodo(true);
          }),
        );
      }

      //this.cbox = Main.panel._rightBox;
      if (this.locationLabel === "right") this.cbox = this.panel._rightBox;
      else if (this.locationLabel === "left") this.cbox = this.panel._leftBox;
      else this.cbox = this.panel._centerBox;
      ...
      this.cbox.connect(
        ...
        Lang.bind(
          this,
          Lang.bind(this, function (actor, m) {
            if (this.autohide_rs && !this.h) {
              this._rshideTimeoutId = Mainloop.timeout_add_seconds(
                this.autohide_rs_time,
                Lang.bind(this, function () {
                  if (!this.h) {
                    //this.h=true;
                    this.doAction(true);
  ...
  doAction: function (updalreadyH) {
    ...
    if (this.h) {
      ...
      for (let i = p - 1; i > -1; i--) {
        ...
        if (
          _children[i]._applet._uuid == "systray@cinnamon.org" ||
          _children[i]._applet._uuid == "systray@cinnaman"
        ) {
          ...
          _children[i]._applet._uuid == "separator@cinnamon.org"
        ) {
          break;
        }
        _children[i].hide();
      ...
      for (let i = 0; i < p; i++) {
        if (this.alreadyH.indexOf(_children[i]) < 0) _children[i].show();
  ...
  on_panel_edit_mode_changed: function () {
    this.panelEditMode.setToggleState(
      global.settings.get_boolean("panel-edit-mode"),
    );
